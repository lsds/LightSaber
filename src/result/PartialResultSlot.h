...
struct alignas(64) PartialResultSlotWithoutFragments {
  ...
  PartialResultSlotWithoutFragments(int index = -1)
      : m_index(index),
        m_numberOfResults(0),
        m_slot(-1),
  ...
  void setPreviousSlot(int prev) {
    if (prev != 1 && prev != 3) {
      throw std::runtime_error("error: setting the previous slot value to " +
                               std::to_string(prev));
    }
    m_previousSlot = prev;
  }
  ...
  int getPreviousSlot() {
    if (m_previousSlot != 1) {
      throw std::runtime_error("error: getting the previous slot value " +
                               std::to_string(m_previousSlot));
    }
...
/*
 * \brief PartialResultSlots are used for operators with window fragments and
 * single-threaded merge phase
 *
 */

struct alignas(64) PartialResultSlot {
  std::shared_ptr<PartialWindowResults> m_closingWindows, m_pendingWindows,
      m_openingWindows, m_completeWindows;
  std::atomic<int> m_slot;
  int m_index;
  long m_freePointer;
  int m_latch;
  ...
  PartialResultSlot(int index = -1)
      : m_closingWindows(nullptr),
  ...
  void connectTo(PartialResultSlot *nextSlot) { m_next = nextSlot; }
  ...
  void release() {
    if (m_closingWindows != nullptr) {
      PartialWindowResultsFactory::getInstance().free(
          m_closingWindows->getThreadId(), m_closingWindows);
      m_closingWindows.reset();
    }
    if (m_openingWindows != nullptr) {
      PartialWindowResultsFactory::getInstance().free(
          m_openingWindows->getThreadId(), m_openingWindows);
      m_openingWindows.reset();
    }
    if (m_pendingWindows != nullptr) {
      PartialWindowResultsFactory::getInstance().free(
          m_pendingWindows->getThreadId(), m_pendingWindows);
  ...
  bool releaseEmptyPartials() {
    if (m_closingWindows != nullptr &&
        m_closingWindows->numberOfWindows() == 0) {
      PartialWindowResultsFactory::getInstance().free(
          m_closingWindows->getThreadId(), m_closingWindows);
      m_closingWindows.reset();
    }
  ...
  bool tryRelease() {
    ...
    if (m_closingWindows != nullptr) {
      if (m_closingWindows->numberOfWindows() == 0 ||
        ...
        if (debug)
          std::cout << m_closingWindows.use_count()
                    << " closing windows with still a reference\n";
      }
    } else {
      cnt++;
    }
    if (m_openingWindows != nullptr) {
      if (m_openingWindows->numberOfWindows() == 0 ||
          m_openingWindows.use_count() == 1) {
        cnt++;
        PartialWindowResultsFactory::getInstance().free(
            m_openingWindows->getThreadId(), m_openingWindows);
        m_openingWindows.reset();
        ...
        if (debug)
          std::cout << m_openingWindows.use_count()
                    << " opening windows with still a reference\n";
      }
    } else {
      cnt++;
    }
    if (m_pendingWindows != nullptr) {
      if (m_pendingWindows->numberOfWindows() == 0 ||
          m_pendingWindows.use_count() == 1) {
        cnt++;
        PartialWindowResultsFactory::getInstance().free(
            m_pendingWindows->getThreadId(), m_pendingWindows);
        m_pendingWindows.reset();
  ...
  /*
   * Aggregate this node's opening windows with node p's closing or pending
   * windows. The output of this operation will always produce complete or
   * opening windows - never pending and never closing ones.
   */
  void aggregate(PartialResultSlot *partialSlot,
                 AggregateOperatorCode *aggrOperator) {
    if (m_openingWindows->isEmpty()) { /* Nothing to aggregate */
      if ((!partialSlot->m_closingWindows->isEmpty()) ||
          (!partialSlot->m_pendingWindows->isEmpty())) {
        throw std::runtime_error(
            "error: there are no opening windows but next slot has closing or "
            "pending windows");
      }
    ...
    if (partialSlot->m_closingWindows->isEmpty() &&
        partialSlot->m_pendingWindows->isEmpty()) {
      throw std::runtime_error(
          "error: there are opening windows but next slot has neither closing "
          "nor pending windows");
    }
    /*
     ...
     */
    if (debug) {
      std::cout << "[DBG] aggregate "
                << std::to_string(m_openingWindows->getPosition()) << " bytes ("
                << std::to_string(m_openingWindows->numberOfWindows())
                << " opening windows) with "
                << std::to_string(partialSlot->m_closingWindows->getPosition())
                << " bytes ("
                << std::to_string(
                       partialSlot->m_closingWindows->numberOfWindows())
                << " closing windows)" << std::endl;
    }
    ...
    if (numOfClosingWindows > 0) {
      if (numOfOpeningWindows < numOfClosingWindows)
        throw std::runtime_error(
            "error: closing window partial results are more then the opening "
            "ones");

      aggrOperator->aggregatePartials(
          m_openingWindows, partialSlot->m_closingWindows, m_completeWindows,
          numOfClosingWindows, pos, tupleSize, true);
      m_completeWindows->setPosition(pos);
      m_completeWindows->incrementCount(numOfClosingWindows);
      partialSlot->m_closingWindows->nullify();
    }

    /* There may be some opening windows left, in which case they are aggregated
     * with node partialSlot's pending one. The result will be stored
     * (prepended) in partialSlot's opening windows */
    auto remainingWindows = numOfOpeningWindows - numOfClosingWindows;
    if (remainingWindows) {
      if (numOfPendingWindows != 1) {
        throw std::runtime_error(
            "error: there are opening windows left but next slot has no "
            "pending windows");
      }
      if (debug) {
        std::cout << "[DBG] aggregate " << std::to_string(remainingWindows)
                  << " remaining opening windows with pending" << std::endl;
      }
      int nextOpenWindow = numOfClosingWindows;

      aggrOperator->aggregatePartials(
          m_openingWindows, partialSlot->m_pendingWindows, m_completeWindows,
          remainingWindows, pos, tupleSize, false);
      /* Prepend this opening windows (starting from `nextOpenWindow`) to node
       * partialSlot's opening windows. We have to shift the start pointers of
       * partialSlot's opening windows down. There are `count` new windows. The
       * window size equal the hash table size or a single tuple if we don't
       * have group by:
       */
      auto windowSize = (aggrOperator->hasGroupBy())
      ...
      partialSlot->m_openingWindows->prepend(m_openingWindows.get(),
                                             nextOpenWindow, remainingWindows,
                                             windowSize, tupleSize);
      partialSlot->m_pendingWindows->nullify();
    }
    m_openingWindows->nullify();
  }
  ...
  int getNumberOfWindowFragments(bool hasPtrs = true) {
    ...
    if (hasPtrs) {
      ...
      fragments +=
          (m_completeWindows && m_completeWindows->numberOfWindows() > 0);
      fragments = (fragments >= 1) ? 1 : 0;
    } else {
      fragments +=
          (m_closingWindows && m_closingWindows->numberOfWindows() > 0);
      fragments +=
          (m_pendingWindows && m_pendingWindows->numberOfWindows() > 0);
      fragments +=
          (m_openingWindows && m_openingWindows->numberOfWindows() > 0);
      fragments +=
          (m_completeWindows && m_completeWindows->numberOfWindows() > 0);
    }
...
struct PartialWindowResultsWrapper {
  ...
  PartialWindowResultsWrapper(
      std::shared_ptr<PartialWindowResults> partialWindows = nullptr,
      int pos = -1, bool isClosing = false)
      : m_partialWindows(partialWindows),
        m_windowPos(pos),
        m_isClosing(isClosing){};
};

struct CircularList {
  std::vector<PartialWindowResultsWrapper,
              tbb::cache_aligned_allocator<PartialWindowResultsWrapper>>
      m_buffer;
  int m_size;
  int m_readIdx;
  int m_writeIdx;
  int m_elements = 0;
  CircularList(int size = 0)
      : m_buffer(size, PartialWindowResultsWrapper()), m_size(size) {
    m_readIdx = 0;
    m_writeIdx = size - 1;
  }
  ...
  void push_back(std::shared_ptr<PartialWindowResults> partialWindows,
    ...
    m_writeIdx++;
    if (m_writeIdx == (int)m_buffer.size()) m_writeIdx = 0;
  ...
  void pop_front() {
    m_elements--;
    m_readIdx++;
    if (m_readIdx == (int)m_buffer.size()) m_readIdx = 0;
  }
...
/*
 * \brief PartialWindowResultSlot are used for operators with window fragments
 * and multi-threaded merge phase
 *
 */

struct alignas(64) PartialWindowResultSlot {
  // tbb::concurrent_queue<std::shared_ptr<PartialWindowResultsWrapper>>
  // partialWindows;
  ...
  PartialWindowResultSlot(int index = -1)
      : m_completeWindows(nullptr),
        m_slot(-1),
        m_finalize(false),
        m_hasComplete(false),
  ...
  void release(int qid, bool isMostDownstream, QueryBuffer &freeBuffer,
               std::atomic<int> &nextToForward, int numberOfSlots,
               std::vector<PartialResultSlot *> &slotsToRelease) {
    if (!m_resSlots.empty()) {
      if (slotsToRelease.empty()) {
        ...
        for (auto slot : m_resSlots) {
          if (slot->m_slot.load() == 3) {
            ...
            // if (!success)
            //    throw std::runtime_error("error: invalid state for
            //    PartialResultSlot while freeing");
            /* Free input buffer */
            auto fPointer = slot->m_freePointer;
            if (fPointer != INT_MIN) freeBuffer.free(fPointer);
            // std::cout << "[DBG] setting slot " +
            // std::to_string(slot->m_index) + " to -1" << std::endl;
  ...
  void aggregate(AggregateOperatorCode *aggrOperator, int pid) {
    ...
    if (m_completeWindows == nullptr ||
      ...
      m_completeWindows =
          PartialWindowResultsFactory::getInstance().newInstance(
              pid, aggrOperator->getHashTableSizeAfterCodeGeneration(), 2);
      m_completeWindows->getStartPointers()[0] = 0;
      m_completeWindows->getStartPointers()[1] =
          m_hashTableSize;  // store the final result in the first half
      // m_completeWindows->getStartPointers()[2] = hashTableSize * 2; // store
      // intermediate results in the second half
    ...
    while ((partialWindowResult = peekNextWindow(window, isClosing)) !=
           nullptr) {
      if (!isClosing) {
        // aggrOperator->aggregateSinglePartial(m_completeWindows,
        // partialWindowResult, window, pos, tupleSize, false);
      } else {
        // pack results and finalize the window
        // aggrOperator->aggregateSinglePartial(m_completeWindows,
        // partialWindowResult, window, pos, tupleSize, true);
        m_completeWindows->setPosition(pos);
  ...
  void prefetch() {
    ...
    for (int i = 0; i < m_partialWindows.m_elements; ++i) {
      ...
      __builtin_prefetch(m_partialWindows.m_buffer[idx]
                             .m_partialWindows->getBufferPtrs()[window],
                         1, 3);
      idx++;
      if (idx == m_partialWindows.size()) idx = 0;
    }
  }
  ...
  void aggregateAll(AggregateOperatorCode *aggrOperator, int pid) {
    ...
    for (int i = 0; i < size; ++i) {
      ...
      if (pw->m_partialWindows == nullptr)
        throw std::runtime_error(
            "error: the next partial window for aggregation is null");
      if (i == 0) {
        // memcpy the first hashtable for simplicity
        aggrOperator->aggregateSinglePartial(
            m_completeWindows, m_completeWindow, m_completeWindowsStartPos,
    ...
    aggrOperator->aggregateSinglePartial(
        m_completeWindows, m_completeWindow, m_completeWindowsStartPos, nullptr,
        window, startPos, endPos, tupleSize, true);
    m_completeWindowsStartPos = startPos;
    m_length = endPos - startPos;
    m_slot.store(3);
  }

  std::shared_ptr<PartialWindowResults> peekNextWindow(int &windowPos,
                                                       bool &isClosing) {
  ...
  void tryReleaseWindow(
      bool isPending,
      std::shared_ptr<PartialWindowResults> partialWindowResult) {
    if (!isPending || partialWindowResult.use_count() == 1) {
      PartialWindowResultsFactory::getInstance().free(
          partialWindowResult->getThreadId(), partialWindowResult);
    }
  }
  ...
  void setResultSlot(PartialResultSlot *slot) {
    ...
    if (!slot->m_slot.compare_exchange_weak(oldVal, 3)) {
      // std::cout << "[DBG] warning: setting the previous slot value because
      // the state is not equal to 3" << std::endl;
      // todo: does this work??
      slot->setPreviousSlot(3);
      // throw std::runtime_error("error: failed to set the result slot as the
      // previous value was " + std::to_string(oldVal));
    }
    // std::cout << "[DBG] setting slot " + std::to_string(slot->m_index) + " to
    // 3" << std::endl;
  }

  void setOpeningBatchId(int batchId) { m_openingBatchId = batchId; }

  void setClosingBatchId(int batchId) { m_closingBatchId = batchId; }

  void setHasAllBatches() { m_finalize.store(true); }

  void setHasComplete() { m_hasComplete.store(true); }
  ...
  bool hasWork() {
    ...
    if (m_partialWindows.size() == 0) return false;
    return true;
  }
  ...
  void packResult(int tupleSize) {
    ...
    for (currentIndex = 0; currentIndex < (size_t)m_hashTableSize * tupleSize;
         currentIndex += tupleSize) {
      if (completeWindow[currentIndex] == 1) {
        if (indexToInsert != currentIndex) {
          std::memcpy(&completeWindow[indexToInsert],
                      &completeWindow[currentIndex], tupleSize);
        }
  ...
  bool isReady() { return m_hasComplete.load(); }
