...
ResultHandler::ResultHandler(Query &query, QueryBuffer &freeBuffer, bool hasWindowFragments, bool useParallelMerge) :
    m_query(query),
...
void ResultHandler::forwardAndFreeWithoutFrags(WindowBatch *batch) {
  ...
  try {
    auto oldVal = -1;
    while (!m_resultsWithoutFrags[idx].m_slot.compare_exchange_weak(oldVal, 0)) {
      std::cout << "warning: result collector (" << std::this_thread::get_id()
                << ") blocked: query " + std::to_string(currentQuery->getId()) +
                    " task " + std::to_string(taskId) + " slot " + std::to_string(idx) << std::endl;
    ...
    oldVal = 1;
    if (!m_resultsWithoutFrags[m_nextToForward].m_slot.compare_exchange_strong(oldVal, 2)) {
    ...
    while (busy) {
      ...
      if (length > 0 && currentQuery->getNumberOfDownstreamQueries() > 0) {
        ...
        if (SystemConf::getInstance().LATENCY_ON && (m_resultsWithoutFrags[m_nextToForward].m_latencyMark != -1)) {
          long t1 =
              m_resultsWithoutFrags[m_nextToForward].m_latencyMark;//(long) Utils::getSystemTimestamp (freeBuffer.getLong (resultsWithoutFrags[m_nextToForward].latencyMark));
        ...
        for (int q = nextQuery; q < currentQuery->getNumberOfDownstreamQueries(); ++q) {
          if (currentQuery->getDownstreamQuery(q) != nullptr) {
            bool success = currentQuery->getDownstreamQuery(q)->getTaskDispatcher()->
                tryDispatch(buffer->getBuffer().data(), length, m_resultsWithoutFrags[m_nextToForward].m_latencyMark);
            if (!success) {
              std::cout << "[DBG] failed to forward results to next query..." << std::endl;
      ...
      if (currentQuery->isMostDownstream()) {
        if (SystemConf::getInstance().LATENCY_ON && (m_resultsWithoutFrags[m_nextToForward].m_latencyMark != -1)) {
          m_query.getLatencyMonitor().monitor(m_freeBuffer, m_resultsWithoutFrags[m_nextToForward].m_latencyMark);
        }
      }
      ...
      /* Free input buffer */
      int fPointer = m_resultsWithoutFrags[m_nextToForward].m_freePointer;
      if (fPointer != INT_MIN)
        m_freeBuffer.free(fPointer);

      /* Release the current slot */
      m_resultsWithoutFrags[m_nextToForward].m_slot.store(-1);

      /* Increment next */
      m_nextToForward = m_nextToForward + 1;
      m_nextToForward = m_nextToForward % m_numberOfSlots;
...
void ResultHandler::aggregateAndForwardAndFree(WindowBatch *batch) {
  ...
  try {
    ...
    while (!m_results[idx].m_slot.compare_exchange_weak(oldVal, 0)) {
      std::cout << "warning: result collector (" << std::this_thread::get_id()
                << ") blocked: query " + std::to_string(currentQuery->getId()) +
                    " task " + std::to_string(taskId) + " m_slot " + std::to_string(idx) << std::endl;
      std::this_thread::sleep_for(std::chrono::nanoseconds(1));
    }
    ...
    if (m_mergeLock.try_lock()) {
      ...
      while (true) {
        ...
        if (currentSlotFlag > 1)
          throw std::runtime_error("error: inconsistent state in next result slot to aggregate");
        if (currentSlotFlag < 1)
          break;
        ...
        if (!currentSlot->isReady())
          throw std::runtime_error("error: result slot aggregated but is not ready");
    ...
    while (busy) {
      ...
      if (length > 0 && currentQuery->getNumberOfDownstreamQueries() > 0) {
        /* Forward the latency mark downstream... */
        if (SystemConf::getInstance().LATENCY_ON && (m_results[m_nextToForward].m_latencyMark != -1)) {
          long t1 =
              m_results[m_nextToForward].m_latencyMark; //(long) Utils::getSystemTimestamp (freeBuffer.getLong (results[m_nextToForward].latencyMark));
          long t2 = (long) Utils::getTupleTimestamp(buffer->getLong(0));
          buffer->putLong(0, Utils::pack(t1, t2));
        }
        ...
        for (int q = nextQuery; q < currentQuery->getNumberOfDownstreamQueries(); ++q) {
          if (currentQuery->getDownstreamQuery(q) != nullptr) {
            bool success =
                currentQuery->getDownstreamQuery(q)->getTaskDispatcher()->tryDispatch(buffer->getBuffer().data(),
                                                                                      length,
      ...
      if (currentQuery->isMostDownstream()) {
        if (SystemConf::getInstance().LATENCY_ON && (m_results[m_nextToForward].m_latencyMark != -1)) {
          m_query.getLatencyMonitor().monitor(m_freeBuffer, m_results[m_nextToForward].m_latencyMark);
        }
      }
      ...
      /* Free input buffer */
      int fPointer = m_results[m_nextToForward].m_freePointer;
      if (fPointer != INT_MIN)
        m_freeBuffer.free(fPointer);
...
void ResultHandler::aggregateWindowsAndForwardAndFree(WindowBatch *batch) {
  int pid = batch->getOpeningWindows()->getThreadId();
  int taskId = batch->getTaskId();
  auto currentQuery = batch->getQuery();
  ...
  try {
    ...
    while (!m_results[taskIdx].m_slot.compare_exchange_weak(oldVal, 0)) {
      std::cout << "warning: result collector (" << std::this_thread::get_id()
                << ") blocked: query " + std::to_string(currentQuery->getId()) +
                    " task " + std::to_string(taskId) + " slot " + std::to_string(taskIdx) << std::endl;
      std::this_thread::sleep_for(std::chrono::nanoseconds(1));
    }
      ...
      while (true) {
        ...
        if (currentSlotFlag != 1 ||
            (currentSlot->m_openingWindows->numberOfWindows() +
                ...
                m_numberOfWindowSlots - m_reservedSlots.load())) {
          break;
        }
    ...
    oldVal = m_windowResults[m_nextToForward].m_slot.load();
    if (oldVal < 3) {
      m_forwardLock.unlock();
      return;
    }
    ...
    while (busy) {
      ...
      if (debug) {
        std::cout << "[DBG] ForwardAndFree " << std::this_thread::get_id() << " start forwarding: " <<
                  m_nextToForward << " nextToForward " << std::endl;
      }

      auto &buffer = m_windowResults[m_nextToForward].m_completeWindows;
      int startPos = m_windowResults[m_nextToForward].m_completeWindowsStartPos;
      int length = m_windowResults[m_nextToForward].m_length;
      int type = buffer->getType();

      // Forward m_windowResults
      if (length > 0 && currentQuery->getNumberOfDownstreamQueries() > 0) {
        // Forward the latency mark downstream...
        if (SystemConf::getInstance().LATENCY_ON && (m_windowResults[m_nextToForward].m_latencyMark != -1)) {
          if (type == 1)
            throw std::runtime_error("Latency is not supported for type 1 buffer yet.");
          long t1 =
              m_windowResults[m_nextToForward].m_latencyMark; //(long) Utils::getSystemTimestamp (freeBuffer.getLong (m_windowResults[m_nextToForward].latencyMark));
          long t2 = (long) Utils::getTupleTimestamp(buffer->getLong(0));
          buffer->putLong(0, Utils::pack(t1, t2));
        }

        int nextQuery = m_windowResults[m_nextToForward].m_latch;
        for (int q = nextQuery; q < currentQuery->getNumberOfDownstreamQueries(); ++q) {
          if (currentQuery->getDownstreamQuery(q) != nullptr) {
            bool success;
            if (type == 0) {
              success = currentQuery->getDownstreamQuery(q)->getTaskDispatcher()->tryDispatch(
                  buffer->getBuffer().data() + startPos, length,
                  m_windowResults[m_nextToForward].m_latencyMark);
            } else {
              //success = currentQuery->getDownstreamQuery(q)->getTaskDispatcher()->tryDispatch(buffer->getBufferPtrs()[0], length);
              throw std::runtime_error("Forwarding for type 1 buffer is not supported yet.");
            }
            if (!success) {
              std::cout << "[DBG] ForwardAndFree: failed to forward results to next query..." << std::endl;
              m_windowResults[m_nextToForward].m_latch = q;
              m_windowResults[m_nextToForward].m_slot.store(oldVal);
              m_forwardLock.unlock();
              return;
            }
          }
      ...
      if (currentQuery->isMostDownstream()) {
        if (SystemConf::getInstance().LATENCY_ON && (m_windowResults[m_nextToForward].m_latencyMark != -1)) {
          m_query.getLatencyMonitor().monitor(m_freeBuffer, m_windowResults[m_nextToForward].m_latencyMark);
        }
      }
      ...
      /* Release the current slot */
      m_windowResults[m_nextToForward].release(m_freeBuffer);
      m_reservedSlots--;

      /* Increment next */
      m_nextToForward = m_nextToForward + 1;
      if (m_nextToForward == m_numberOfWindowSlots)
        m_nextToForward = 0;
      m_forwardedWindows++;
