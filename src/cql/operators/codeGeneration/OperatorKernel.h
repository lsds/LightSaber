...
class OperatorKernel : public OperatorCode, public AggregateOperatorCode {
 private:
  ...
  int m_keyLength = 0, m_valueLength = 0;
  bool m_groupBy;
 ...
 public:
  ...
  void setupWithoutAggregation() {
    ...
    if (m_genCode) {
      ...
      std::string generatedPath = Utils::GetCurrentWorkingDir() + "/GeneratedCode_" + std::to_string(m_id) + ".cpp";
      ...
      llvm::Expected<std::function<void(int, char *, long, long, char *, long &)>> processFn =
          m_codeGen.getFunction<void(int, char *, long, long, char *, long &)>("process");
  ...
  void setupWithAggregation() {
    ...
    if (m_genCode) {
      ...
      if (!hasGroupBy() || !m_usePtrs) { // use the actual values stored sequentially in a buffer to aggregate
        auto processFn =
            m_codeGen.getFunction<void(int, char *, long, long, long, long, long *,
                                       long *, char *, char *, char *, char *, int *,
                                       int *, int *, int *, //long *, long *, long *, long *,
                                       long, int *, char *)>("process");
        auto aggregateFn =
            m_codeGen.getFunction<long(char *, int *, int, char *, int *, int, int, int, bool, char *, long, int &)>(
                "aggregate");
        ...
        auto processFn =
            m_codeGen.getFunction<void(int, char *, long, long, long, long, long *,
                                       ...
                                       int *, int *, int *, //long *, long *, long *, long *,
                                       long, int *, char *)>("process");
        auto computeSizeFn = m_codeGen.getFunction<long()>("getHashTableSizeInBytes");
        ...
        if (!m_useParallelMerge) {
          auto aggregateFn =
              m_codeGen.getFunction<long(char **, int *, int, char **, int *, int,
                                         int, int, bool, char *, long, int &)>("aggregate");
          if (!aggregateFn) {
            std::cout << "Failed to fetch the pointers." << std::endl;
            exit(1);
          }
          ...
          auto aggregateFn =
              m_codeGen.getFunction<long(char *, int, int, char **, int, int &, int &, int &, bool)>("aggregate");
          if (!aggregateFn) {
            std::cout << "Failed to fetch the pointers." << std::endl;
            exit(1);
          }
  ...
  void processWithoutFragments(std::shared_ptr<WindowBatch> batch, Task &task, int pid) {
    ...
    process(pid, inputBuffer->getBuffer().data(), startP, endP, outputBuffer->getBuffer().data(), pos);
  ...
  void processWithFragments(std::shared_ptr<WindowBatch> batch, Task &task, int pid) {
    ...
    if (!hasGroupBy() || !m_usePtrs) { // use the actual values stored sequentially in a buffer to aggregate
      processFragments(pid, buffer.data(), circularBufferSize, startPointer, endPointer, timestampFromPrevBatch,
                       batch->getWindowStartPointers().data(), batch->getWindowEndPointers().data(),
                       openingWindows->getBuffer().data(), closingWindows->getBuffer().data(),
                       pendingWindows->getBuffer().data(), completeWindows->getBuffer().data(),
                       openingStartPointers.data(), closingStartPointers.data(), pendingStartPointers.data(),
                       completeStartPointers.data(),
          //openingWindowIds.data(), closingWindowIds.data(), pendingWindowIds.data(),
          //completeWindowIds.data(),
                       streamStartPointer, &pointersAndCounts[0], m_staticBuffer);
    } else { // use pointers to hashtables to aggregate
      processFragmentsWithPtrs(pid, buffer.data(), circularBufferSize, startPointer, endPointer, timestampFromPrevBatch,
                               batch->getWindowStartPointers().data(), batch->getWindowEndPointers().data(),
                               openingWindows->getBufferPtrs().data(), closingWindows->getBufferPtrs().data(),
                               pendingWindows->getBufferPtrs().data(), completeWindows->getBuffer().data(),
                               openingStartPointers.data(), closingStartPointers.data(), pendingStartPointers.data(),
                               completeStartPointers.data(),
          //openingWindowIds.data(), closingWindowIds.data(), pendingWindowIds.data(),
          //completeWindowIds.data(),
  ...
  bool hasHavingPredicate() const {
    return (m_havingPredicate != nullptr);
  }
 ...
 private:
  ...
  std::string getQueryDefinitionString() {
    ...
    s.append("#define BUFFER_SIZE      " + std::to_string(SystemConf::getInstance().CIRCULAR_BUFFER_SIZE) + "L\n");
    ...
    if (!hasGroupBy()) {
      ...
      s.append("#define MAP_SIZE         " + std::to_string(SystemConf::getInstance().HASH_TABLE_SIZE) + "L\n");
    ...
    if (hasAggregation()) {
      ...
      if (!hasGroupBy()) {
        ...
        if (hasIncremental()) {
          ...
          closingWindows = "aggrStructures[pid].setIntermValues(PARENTS_SIZE-2);\n";
          openingWindows = "aggrStructures[pid].insertSlices();\n"
                           "aggrStructures[pid].setIntermValues(numberOfOpeningWindows);\n";
          pendingWindows = "aggrStructures[pid].setIntermValues(-1);\n";
        }
  ...
  std::string getHashTableMergeString() {
    ...
    s.append(
        ...
        addTabs(completeResPredicate, 4) +
            ...
            "                    openingWindowsRes[posInRes + resultIndex].state = 1;\n"
            "                    /* Put timestamp */\n"
            "                    openingWindowsRes[posInRes + resultIndex].timestamp = openingWindowsRes[posInRes + resultIndex].timestamp;\n"
            "                    /* Put key and value(s) */\n"
  ...
  std::string getHashTableMergeWithPtrsString() {
    ...
    s.append(
        ...
        addTabs(completeResPredicate, 4) +
            ...
            "                    openingWindowsRes[wid][posInRes].state = 1;\n"
            "                    /* Put timestamp */\n"
            "                    openingWindowsRes[wid][posInRes].timestamp = openingWindowsRes[wid][posInRes].timestamp;\n"
            "                    /* Put key and value(s) */\n"
  ...
  std::string getHashTableParallelMergeWithPtrsString() {
    ...
    s.append(
        ...
        "                tempCompleteWindowsRes[posInLeft].state = 1;\n"
        "                /* Put timestamp */\n"
        "                tempCompleteWindowsRes[posInLeft].timestamp = tempCompleteWindowsRes[posInLeft].timestamp;\n"
        "                /* Put key and value(s) */\n"
  ...
  std::string getSingleKeyMergeString() {
    ...
    s.append(
        ...
        "            /* Copy tuple based on output schema */\n"
        "            /* Put timestamp */\n"
        "            completeWindowsRes[resultIndex].timestamp = openingWindowsRes[start1].timestamp;\n"
        "            /* Put value(s) */\n");
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\tcompleteWindowsRes[resultIndex]._" + std::to_string((i + 1)) + " = " +
  ...
  std::string getWriteCompleteResults() {
    ...
    if ((int) m_aggregationTypes->size() > m_outputSchema->numberOfAttributes())
      throw std::runtime_error(
          "error: the number of aggregation types should be <= to the attributes of the output schema");
    std::string s;
    s.append("completeWindowsResults[completeWindowsPointer].timestamp = data[bufferPtr-1].timestamp;\n");
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
  ...
  std::string getWriteCompleteResultsGroupBy() {
    ...
    std::string s;
    s.append(
        "completeWindowsResults[completeWindowsPointer].timestamp = aggrStructures[pid].getBuckets()[i].timestamp;\n");
