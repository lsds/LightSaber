...
class OperatorKernel : public OperatorCode, public AggregateOperatorCode {
 private:
  ...
  std::function<void(int, char *, long, long, long, long, long *, long *,
                     ...
                     /*long *, long *, long *, long * ,*/ long, int *, char *)>
      processFragments;
  std::function<void(int, char *, long, long, long, long, long *, long *,
  ...
  std::function<long(char *, int *, int, char *, int *, int, int, int, bool,
                     char *, long, int &)>
      aggregate;
  std::function<long(char **, int *, int, char **, int *, int, int, int, bool,
                     char *, long, int &)>
 ...
 public:
  OperatorKernel(bool genCode = true, bool usePtrs = true,
                 bool useParallelMerge = false, bool generateFile = true)
  ...
  void setQueryId(int qId) { m_id = qId; }
  ...
  void setProjection(Projection *projection) {
    if (m_expressions != nullptr)
      throw std::runtime_error(
          "error: projection has already been set, try setting static join "
          "after projection");
  ...
  void setSelection(Selection *selection) {
    ...
    if (m_outputSchema == nullptr) m_outputSchema = m_inputSchema;
  }
  ...
  void setCollisionBarrier(int barrier) { m_collisionBarrier = barrier; }
  ...
  void setupWithoutAggregation() {
    ...
    std::ofstream out("GeneratedCode_" + std::to_string(m_id) + ".cpp");
    out << s;
    out.close();

    if (m_genCode) {
      int argc = 2;
      std::string mainPath = Utils::GetCurrentWorkingDir() + "/LightSaber";
      std::string generatedPath = Utils::GetCurrentWorkingDir() +
                                  "/GeneratedCode_" + std::to_string(m_id) +
                                  ".cpp";
      ...
      argv[0] = str0;
      argv[1] = str1;
      m_codeGen.parseAndCodeGen(argc, argv);

      llvm::Expected<
          std::function<void(int, char *, long, long, char *, long &)>>
          processFn =
              m_codeGen
                  .getFunction<void(int, char *, long, long, char *, long &)>(
                      "process");
    ...
    if (m_genCode) {
      ...
      if (!hasGroupBy() ||
        ...
        auto processFn = m_codeGen.getFunction<void(
            int, char *, long, long, long, long, long *, long *, char *, char *,
        ...
        if (!m_useParallelMerge) {
          auto aggregateFn =
              m_codeGen
                  .getFunction<long(char **, int *, int, char **, int *, int,
                                    int, int, bool, char *, long, int &)>(
                      "aggregate");
          if (!aggregateFn) {
            std::cout << "Failed to fetch the pointers." << std::endl;
  ...
  AggregationType &getAggregationType(int idx) override {
    if (!hasAggregation())
      throw std::runtime_error(
          "error: aggregation operator hasn't been set up");
  ...
  explicit operator std::string() const { return toSExpr(); }
  ...
  void processWithoutFragments(const std::shared_ptr<WindowBatch> &batch,
                               Task &task, int pid) {
  ...
  void processWithFragments(const std::shared_ptr<WindowBatch> &batch,
                            Task &task, int pid) {
    auto circularBuffer = batch->getInputQueryBuffer();
    auto circularBufferSize =
        circularBuffer->getBufferCapacity(task.getNumaNodeId());
    ...
    auto openingWindows =
        (!hasGroupBy() || !m_usePtrs)
            ? PartialWindowResultsFactory::getInstance().newInstance(pid)
            : PartialWindowResultsFactory::getInstance().newInstance(
                  pid, m_hashTableSize);
    auto closingWindows =
        (!hasGroupBy() || !m_usePtrs)
            ? PartialWindowResultsFactory::getInstance().newInstance(pid)
    ...
    if (m_debug) {
      std::cout << "[DBG] processWithFragments [TID " << task.getTaskId()
                << "]:"
                << " pipeline " << m_id << " entering generated function with "
                ...
                << circularBufferSize << " circularBufferSize " << std::endl;
    }

    if (!hasGroupBy() || !m_usePtrs) {  // use the actual values stored
                                        // sequentially in a buffer to aggregate
      processFragments(
          pid, buffer.data(), circularBufferSize, startPointer, endPointer,
          timestampFromPrevBatch, batch->getWindowStartPointers().data(),
          batch->getWindowEndPointers().data(), openingWindows->getBufferRaw(),
          closingWindows->getBufferRaw(), pendingWindows->getBufferRaw(),
          ...
          // openingWindowIds.data(), closingWindowIds.data(),
          // pendingWindowIds.data(), completeWindowIds.data(),
    ...
    if (m_debug) {
      std::cout << "[DBG] processWithFragments [TID " << task.getTaskId()
                << "]:"
                << " pipeline " << m_id << " exiting generated function with "
                << batch->getBatchStartTimestamp() << " startTimestamp "
                << batch->getBatchEndTimestamp() << " endTimestamp "
                << streamStartPointer << " streamStartPointer "
  ...
  void aggregatePartials(
      std::shared_ptr<PartialWindowResults> openingWindows,
      std::shared_ptr<PartialWindowResults> closingOrPendingWindows,
      std::shared_ptr<PartialWindowResults> completeWindows, int numOfWindows,
      long &windowsPos, int &tupleSize, bool pack) override {
    if (!hasAggregation())
      throw std::runtime_error(
          "error: aggregation operator hasn't been set up");
    ...
    if (!hasGroupBy() || !m_usePtrs) {  // use the actual values stored
      ...
      if (pack) {
        windowsPos =
            aggregate(openingWindows->getBufferRaw(),
                      ...
                      (int)closingOrPendingWindows->getPosition(), 0,
                      numOfWindows, pack, completeWindows->getBufferRaw(),
                      completeWindows->getPosition(), tupleSize);
      } else {
        windowsPos =
            aggregate(openingWindows->getBufferRaw(),
                      openingWindows->getStartPointers().data(),
      ...
      if (pack) {
        windowsPos = aggregateWithPtrs
            ...
            (openingWindows->getBufferPtrs().data(),
             ...
             closingOrPendingWindows->getStartPointers().data(),
             (int)closingOrPendingWindows->getPosition(), 0, numOfWindows, pack,
             completeWindows->getBufferRaw(), completeWindows->getPosition(),
             tupleSize);
      } else {
        windowsPos = aggregateWithPtrs
            // aggregate2
            (openingWindows->getBufferPtrs().data(),
             ...
             closingOrPendingWindows->getStartPointers().data(),
             (int)closingOrPendingWindows->getPosition(),
             openingWindows->numberOfWindows() - numOfWindows, numOfWindows,
             pack, completeWindows->getBufferRaw(),
             completeWindows->getPosition(), tupleSize);
      }
    }
  }

  void aggregateSinglePartial(
      std::shared_ptr<PartialWindowResults> completeWindows, int completeWindow,
      int completeWindowsStartPos,
      std::shared_ptr<PartialWindowResults> partialWindows, int window,
      int &startPos, int &endPos, int &tupleSize, bool pack) override {
    if (!hasAggregation() || !m_useParallelMerge || !hasGroupBy())
      throw std::runtime_error(
          "error: aggregation operator hasn't been set up for parallel merge");
    ...
    if (!hasGroupBy() || !m_usePtrs) {  // use the actual values stored
      ...
      aggregateSingleHashTableWithPtrs(
          completeWindows->getBufferRaw(), completeWindow,
          completeWindowsStartPos,
          (partialWindows != nullptr) ? partialWindows->getBufferPtrs().data()
                                      : nullptr,
          window, startPos, endPos, tupleSize, pack);
    }
  }
  ...
  bool hasProjection() const { return (m_expressions != nullptr); }

  bool hasSelection() const { return (m_predicate != nullptr); }

  bool hasHavingPredicate() const { return (m_havingPredicate != nullptr); }

  bool hasAggregation() const {
    if (m_aggregationTypes != nullptr && m_aggregationAttributes != nullptr &&
        m_aggregationTypes->size() != m_aggregationAttributes->size())
      throw std::runtime_error(
          "error: the number  of aggregation types should ne equal to the "
          "aggregation attributes");
    return (m_aggregationTypes != nullptr ||
            m_aggregationAttributes != nullptr);
  }

  bool hasStaticHashJoin() const {
 ...
 private:
  std::string tab = "\t";
  std::string newline = "\n";
  std::string getCombineFunction(AggregationType type, std::string leftArg,
                                 std::string rightArg) {
    if (type == CNT || type == SUM)
      ...
      return "std::make_pair(" + leftArg + ".first+" + rightArg + ".first," +
             leftArg + ".second+" + rightArg + ".second)";
    else {
  ...
  std::string getC_Definitions(std::string computation) {
    ...
    s.append(
        "extern \"C\" {\n"
        "void process (int pid, char *inputBuffer, long startPointer, long "
        "endPointer,\n"
  ...
  std::string getAggregate_C_Definitions() {
    ...
    s.append(
        "extern \"C\" {\n"
        "    void process (int pid, char *inputBuffer, size_t inputBufferSize, "
        "long startPointer, long endPointer, long timestampFromPrevBatch,\n"
        "                              long *windowStartPointers, long "
        "*windowEndPointers, char *" +
        ptr + "openingWindowsBuffer, char *" + ptr +
        "closingWindowsBuffer,\n"
        "                              char *" +
        ptr +
        "pendingWindowsBuffer, char *completeWindowsBuffer,\n"
        "                              int *openingStartPointers, int "
        "*closingStartPointers, int *pendingStartPointers, int "
        "*completeStartPointers,\n"
        //"                              long *openingWindowIds, long
        //*closingWindowIds, long *pendingWindowIds, long *completeWindowIds,\n"
        "                              long streamStartPointer, int "
        "*pointersAndCounts, char *staticBuffer) {\n"
        "            processData (pid, inputBuffer, inputBufferSize, "
        "startPointer, endPointer, timestampFromPrevBatch,\n"
        "            windowStartPointers, windowEndPointers, "
        "openingWindowsBuffer, closingWindowsBuffer,\n"
        "            pendingWindowsBuffer, completeWindowsBuffer,\n"
        "            openingStartPointers, closingStartPointers, "
        "pendingStartPointers, completeStartPointers,\n"
        //"            openingWindowIds, closingWindowIds, pendingWindowIds,
        // completeWindowIds,\n"
    ...
    if (!m_useParallelMerge) {
      s.append(
          "    long aggregate (char *" + ptr +
          "openingBuffer, int *openingStartPointers, int openingEndPointer,\n"
          "                            char *" +
          ptr +
          "closingOrPendingBuffer, int *copStartPointers, int copEndPointer,\n"
          "                            int startingWindow, int numOfWindows, "
          "bool pack,\n"
          "                            char* completeBuffer, long "
          "completeBufferPtr, int &tupleSize) {\n"
          "        return aggregatePartials (openingBuffer, "
          "openingStartPointers, openingEndPointer,\n"
          "                closingOrPendingBuffer, copStartPointers, "
          "copEndPointer,\n"
          "                startingWindow, numOfWindows, pack, completeBuffer, "
          "completeBufferPtr, tupleSize);\n"
          "    };\n" +
          getSize + "}\n");
    } else {
      s.append(
          "    long aggregate (char *completeBuffer, int completeWindow, int "
          "completeStartPos,\n"
          "                            char **partialBuffer, int "
          "partialWindow,\n"
          "                            int &startPos, int &endPos,\n"
          "                            int &tupleSize, bool pack) {\n"
          "        return aggregatePartials (completeBuffer, completeWindow, "
          "completeStartPos, partialBuffer,\n"
  ...
  std::string getAggregationString() const {
    ...
    if (hasAggregation()) {
      ...
      for (int i = 0; i < (int)(*m_aggregationTypes).size(); ++i)
        s.append(AggregationTypes::toString((*m_aggregationTypes)[i]))
            .append("(")
            .append((*m_aggregationAttributes)[i]->toSExpr())
            .append(")")
            .append(" ");
      s.append("(group-by ?")
          .append(" ")
          .append(std::to_string(m_groupBy))
          .append(") ");
      s.append("(incremental ?")
          .append(" ")
          .append(std::to_string(m_processIncremental))
          .append(")");
    }
    s.append("\n");
    return s;
  ...
  std::string getProjectionString() const {
    ...
    if (hasProjection()) {
      ...
      for (auto e : *m_expressions) {
        s.append(e->toSExpr());
        if (i != (int)(*m_expressions).size() - 1) s.append(", ");
        i++;
      }
      s.append(")");
    }
  ...
  std::string getInputSchemaString() {
    ...
    for (int i = 1; i < m_inputSchema->numberOfAttributes(); i++) {
      ...
      switch (type) {
        case BasicType::Integer:
          s.append("\tint _" + std::to_string(i) + ";\n");
          break;
        case BasicType::Float:
          s.append("\tfloat _" + std::to_string(i) + ";\n");
          break;
        case BasicType::Long:
          s.append("\tlong _" + std::to_string(i) + ";\n");
          break;
        case BasicType::LongLong:
          s.append("\t__uint128_t _" + std::to_string(i) + ";\n");
          break;
        default:
          throw std::runtime_error(
              "error: failed to generate tuple struct (attribute " +
  ...
  std::string getIntermediateSchemaString() {
    ...
    auto schema = ExpressionUtils::getTupleSchemaFromExpressions(
        *m_expressions, "IntermediateStream");
    std::string s;
  ...
  std::string getQueryDefinitionString() {
    ...
    s.append("#define WINDOW_SIZE      " +
             std::to_string(m_windowDefinition->getSize()) + "L\n");
    s.append("#define WINDOW_SLIDE     " +
             std::to_string(m_windowDefinition->getSlide()) + "L\n");
    s.append("#define PANES_PER_WINDOW " +
             std::to_string(m_windowDefinition->numberOfPanes()) + "L\n");
    s.append("#define PANES_PER_SLIDE  " +
             std::to_string(m_windowDefinition->panesPerSlide()) + "L\n");
    s.append("#define PANE_SIZE        " +
             std::to_string(m_windowDefinition->getPaneSize()) + "L\n");
    // s.append("#define PARTIAL_WINDOWS
    // "+std::to_string(SystemConf::getInstance().PARTIAL_WINDOWS)+"L\n");
    s.append("#define BUFFER_SIZE      " +
             std::to_string(
                 m_config ? m_config->getCircularBufferSize()
                          : SystemConf::getInstance().CIRCULAR_BUFFER_SIZE) +
             "L\n");
    s.append("#define UNBOUNDED_SIZE   " +
             std::to_string(SystemConf::getInstance().UNBOUNDED_BUFFER_SIZE) +
             "L\n");
    if (!hasGroupBy()) {
      s.append("#define MAP_SIZE         " + std::to_string(1) + "L\n");
    } else {
      s.append("#define MAP_SIZE         " +
               std::to_string(m_config
                                  ? m_config->getHashtableSize()
                                  : SystemConf::getInstance().HASH_TABLE_SIZE) +
               "L\n");
      s.append("#define KEY_SIZE         " + std::to_string(m_keyLength) +
               "L\n");
    }
    s.append("\n");
    return s;
  ...
  std::string getSingleKeyDataStructureString() {
    ...
    AbstractTreeRepresentation tree(*m_windowDefinition, *m_aggregationTypes,
                                    hasNonInvertible());
  ...
  std::string getHashTableBucketString() {
    ...
    for (unsigned long i = 0; i < (*m_aggregationTypes).size(); ++i) {
      switch ((*m_aggregationTypes)[i]) {
        case SUM:
          s.append("\tfloat _" + std::to_string((i + 1)) + ";\n");
          break;
        case AVG:
          s.append("\tfloat _" + std::to_string((i + 1)) + ";\n");
        ...
        case MAX:
          ...
          break;
        default:
          throw std::runtime_error("error: invalid aggregation type");
      }
    }
    s.append("\tValue () {\n");
    for (unsigned long i = 0; i < (*m_aggregationTypes).size(); ++i) {
      switch ((*m_aggregationTypes)[i]) {
        case SUM:
          s.append("\t\t_" + std::to_string((i + 1)) + " = 0.0f;\n");
          break;
        case AVG:
          s.append("\t\t_" + std::to_string((i + 1)) + " = 0.0f;\n");
          // s.append("\t\t_c"+std::to_string((i+1))+" = 0.0f;\n");
        ...
        case CNT:
          ...
          break;
        case MIN:
          s.append("\t\t_" + std::to_string((i + 1)) + " = FLT_MAX;\n");
          break;
        case MAX:
          s.append("\t\t_" + std::to_string((i + 1)) + " = FLT_MIN;\n");
          break;
        default:
          throw std::runtime_error("error: invalid aggregation type");
      }
    ...
    if (m_customHashTable.empty()) {
      if (m_numberOfKeyAttributes == 1 &&
        ...
        s.append("using KeyT = " + key +
                 ...
                 "    constexpr bool operator()(const KeyT& lhs, const KeyT& "
                 "rhs) const {\n"
                 "        return lhs == rhs;\n"
                 "    }\n"
                 "};\n"
      ...
      if (m_collisionBarrier > 0)
        s.append("    int       _barrier     = " +
                 std::to_string(m_collisionBarrier) + ";\n");
      s.append(
          "public:\n"
          "    HashTable ();\n"
          ...
          "    void insert_or_modify (KeyT &key, ValueT &value, long "
          "timestamp);\n"
          "    bool evict (KeyT &key);\n"
          "    void insertSlices ();\n"
          "    void evictSlices ();\n"
          ...
          "        throw std::runtime_error (\"error: the size of the hash "
          "table has to be a power of two\\n\");\n"
          "    }\n"
          "}\n"
          "\n"
          "void HashTable::init () {\n"
          ...
          "    }\n"
          "\n"
          "    _buckets  = (BucketT*)_mm_malloc(_num_buckets * "
          "sizeof(BucketT), 64);\n");
      if (hasIncremental())
        s.append(
            "    _aggrs  = (AggrT*)_mm_malloc(_num_buckets * sizeof(AggrT), "
            "64);\n");
      s.append(
          "    if (!_buckets /*|| !_aggrs*/) {\n"
          "        free(_buckets);\n"
      ...
      s.append(
          ...
          "            _buckets[i].key = key; //std::memcpy(&_buckets[i].key, "
          "key, KEY_SIZE);\n"
          ...
          "}\n"
          "\n"
          "void HashTable::insert_or_modify (KeyT &key, ValueT &value, long "
          "timestamp) {\n"
          "    size_t ind = _hasher(key) & _mask, i = ind;\n"
          "    char tempState;\n");
      ...
      for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
        if ((*m_aggregationTypes)[i] == AVG) {
          s.append("\t\t\t_buckets[i].value._" + std::to_string((i + 1)) +
                   " = " +
                   getCombineFunction(
                       SUM, "_buckets[i].value._" + std::to_string((i + 1)),
          ...
          s.append("\t\t\t_buckets[i].value._" + std::to_string((i + 1)) +
                   " = " +
                   getCombineFunction(
                       (*m_aggregationTypes)[i],
                       "_buckets[i].value._" + std::to_string((i + 1)),
                       "value._" + std::to_string((i + 1))) +
                   ";\n");
      }
      s.append(
          ...
          "        }\n"
          "        if (!tempState && (_eq(_buckets[i].key, key) || "
          "_buckets[i].dirty == 0)) { // first insert -- keep track of "
      ...
      if (m_collisionBarrier > 0)
        s.append(
            "        steps++;\n"
            "        if (steps == _barrier ) {\n"
            "            printf(\"Too many collisions, increase the "
            "size...\\n\");\n"
            "            exit(1);\n"
            "        };\n");
      s.append(
      ...
      if (hasIncremental()) {
        s.append(
            ...
            "    for (auto i = 0; i < _num_buckets; ++i) {\n"
            "        int temp = _aggrs[i].addedElements - "
            "_aggrs[i].removedElements;\n"
            "        if (_buckets[i].state) {\n"
            "                node n;\n");
        for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
          s.append("\t\t\t\tn._" + std::to_string((i + 1)) +
                   " = _buckets[i].value._" + std::to_string((i + 1)) + ";\n");
          if ((*m_aggregationTypes)[i] == AVG) {
            s.append("\t\t\t\tn._c" + std::to_string((i + 1)) +
                     " = _buckets[i].counter;\n");
          }
        }
        s.append(
            ...
            "            node n;\n");
        for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
          s.append("\t\t\tn._" + std::to_string((i + 1)) + " = val._" +
                   std::to_string((i + 1)) + ";\n");
          if ((*m_aggregationTypes)[i] == AVG) {
        ...
        s.append(
            ...
            "    for (auto i = 0; i < _num_buckets; ++i) {\n"
            "        if (_aggrs[i].addedElements - _aggrs[i].removedElements > "
            "0) {\n"
            "            _aggrs[i].evict();\n"
            "        }\n"
            "    }\n"
        ...
        for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
          s.append("\t\t\t_buckets[i].value._" + std::to_string((i + 1)) +
                   " = res._" + std::to_string((i + 1)) + ";\n");
          if ((*m_aggregationTypes)[i] == AVG) {
            s.append("\t\t\t_buckets[i].counter = res._c" +
                     std::to_string((i + 1)) + ";\n");
          }
        }
        s.append(
      ...
      s.append(
          ...
          "    return static_cast<float>(_num_filled) / "
          "static_cast<float>(_num_buckets);\n"
          "}");
    } else {
  ...
  std::string getComputeString() {
    ...
    if (hasAggregation()) {
      ...
      std::string initialiseAggrs, computeAggrs, resetAggrs, insertAggrs,
          evictAggrs, setValues, filter;
      std::string openingWindows, closingWindows, pendingWindows,
          completeWindows;
      std::string resultPointers;
      if (m_windowDefinition->isRangeBased()) filter.append(getSelectionExpr());
      if (!hasGroupBy()) {
        outputBuffers = getAggregationVarsOutputBuffers();
        initialiseAggrs = getAggregationVarsInitialization();
        computeAggrs =
            getAggregationVarsComputation(m_windowDefinition->isRangeBased());
        if (hasIncremental()) {
          ...
          closingWindows =
              "aggrs = "
              "aggrStructures[pid].queryIntermediate(PARENTS_SIZE-2);\n";
          openingWindows =
              "aggrs = "
              "aggrStructures[pid].queryIntermediate(numberOfOpeningWindows);"
              "\n";
          pendingWindows =
        ...
        computeAggrs =
            getGroupByComputation(m_windowDefinition->isRangeBased());
        ...
        if (hasIncremental()) {
          ...
          closingWindows =
              "aggrStructures[pid].setIntermValues(PARENTS_SIZE-2, "
              "(prevClosePane+1) * panesPerSlide + panesPerWindow);\n";
          openingWindows =
              "aggrStructures[pid].insertSlices();\n"
              "aggrStructures[pid].setIntermValues(numberOfOpeningWindows, "
              "prevOpenPane * paneSize);\n"
              "prevOpenPane += panesPerSlide;\n";
          pendingWindows =
              "aggrStructures[pid].setIntermValues(-1, prevPane * paneSize);\n";
        }
      ...
      if (m_windowDefinition->isRowBased()) {
        if (m_windowDefinition->isTumbling()) {
          s.append(getTumblingWindowRows(
              outputBuffers, initialiseAggrs, computeAggrs, resetAggrs,
              openingWindows, closingWindows, pendingWindows, completeWindows,
              resultPointers));
        } else {
          s.append(getSlidingWindowRows(
              outputBuffers, initialiseAggrs, computeAggrs, insertAggrs,
              evictAggrs, resetAggrs, setValues, openingWindows, closingWindows,
              pendingWindows, completeWindows, resultPointers));
        }
      } else {
        if (m_windowDefinition->isTumbling()) {
          s.append(getFillEmptyTumblingWindows(resetAggrs, closingWindows,
                                               completeWindows));
          s.append(getTumblingWindowRange(
              outputBuffers, initialiseAggrs, computeAggrs, resetAggrs,
              openingWindows, closingWindows, pendingWindows, completeWindows,
              resultPointers, filter));
        } else {
          s.append(getFillEmptySlidingWindows(insertAggrs, evictAggrs,
                                              resetAggrs, setValues,
                                              closingWindows, completeWindows));
          s.append(getSlidingWindowRange(
              outputBuffers, initialiseAggrs, computeAggrs, insertAggrs,
              evictAggrs, resetAggrs, setValues, openingWindows, closingWindows,
              pendingWindows, completeWindows, resultPointers, filter));
        }
      }
    } else {
  ...
  std::string getHashTableStaticDeclaration() {
    std::string s;
    s.append("static HashTable aggrStructures[" +
             std::to_string(SystemConf::getInstance().WORKER_THREADS) + "];\n");

    s.append("bool isFirst [" +
             std::to_string(SystemConf::getInstance().WORKER_THREADS) +
             "] = {");
    for (auto i = 0; i < SystemConf::getInstance().WORKER_THREADS; ++i) {
      s.append("true");
      if (i != SystemConf::getInstance().WORKER_THREADS - 1) s.append(", ");
    }
    s.append("};\n");
    s.append("\n");
    return s;
  ...
  std::string getHashTableMergeString() {
    ...
    s.append(
        "long aggregatePartials (char *openingBuffer, int "
        "*openingStartPointers, int openingEndPointer,\n"
        "                      char *closingOrPendingBuffer, int "
        "*copStartPointers, int copEndPointer,\n"
        "                      int startingWindow, int numOfWindows, bool "
        "pack,\n"
        "                      char* completeBuffer, long completeBufferPtr, "
        "int &tupleSize) {\n"
        ...
        "    output_tuple_t *completeWindowsRes = (output_tuple_t *) "
        "completeBuffer; // the results here are packed\n"
        "\n"
        "    // Temp variables for the merging\n"
        "    int resultIndex = (pack) ? "
        "completeBufferPtr/sizeof(output_tuple_t) : startingWindow*mapSize;\n"
        "    int posInB2;\n"
        "    bool isFound;\n"
        "    int posInRes = 0;\n"
        "\n"
        ...
        "        end2 = (pack) ? copStartPointers[wid+1] : "
        "copEndPointer/tupleSize;\n"
        "        if (end1 < 0)\n"
        ...
        "        }\n"
        "        if (start2 == end2) {\n"
        "            printf (\"error: empty closing/pending window partial "
        "result\");\n"
        "            exit(1);\n"
        "        }\n"
        "        // search in the correct hashtables by moving the respective "
        "pointers\n"
        "        HashTable map1 (&openingWindowsRes[resultIndex]);\n"
        "        HashTable map2 (&partialRes[start2]);\n"
        "\n"
        "        if (pack) {\n"
        "            /* Iterate over tuples in first table. Search for key in "
        "the hash table.\n"
        "             * If found, merge the two entries. */\n"
        "            for (int idx = start1; idx < end1; idx++) {\n"
        "                if (openingWindowsRes[idx].state != 1) /* Skip empty "
        "slot */\n"
        "                    continue;\n"
        "                isFound = map2.get_index(openingWindowsRes[idx].key, "
        "posInB2);\n"
        "                if (posInB2 < 0) {\n"
        ...
        "                    /* Put timestamp */\n"
        "                    completeWindowsRes[resultIndex].timestamp = "
        "openingWindowsRes[idx].timestamp;\n"
        "                    /* Put key */\n");
    if (m_numberOfKeyAttributes == 1) {
      s.append(
          "                    completeWindowsRes[resultIndex]._1 = "
          "openingWindowsRes[idx].key;\n");
    } else {
      for (auto i = 0; i < m_numberOfKeyAttributes; ++i) {
        s.append("                    completeWindowsRes[resultIndex]._" +
                 std::to_string(i + 1) + " = openingWindowsRes[idx].key._" +
                 std::to_string(i) + ";\n");
      }
    }
    ...
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\t\t\tcompleteWindowsRes[resultIndex]._" +
                 std::to_string((completeResIndex + 1)) + " = " +
    ...
    s.append(
        ...
        "                } else { // merge values based on the number of "
        "aggregated values and their types!            \n"
        "                    /* Copy tuple based on output schema */\n"
        "                    /* Put timestamp */\n"
        "                    completeWindowsRes[resultIndex].timestamp = "
        "std::max(completeWindowsRes[resultIndex].timestamp, "
        "openingWindowsRes[idx].timestamp);\n"
        "                    /* Put key */\n");
    if (m_numberOfKeyAttributes == 1) {
      s.append(
    ...
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append(
            "\t\t\t\t\tcompleteWindowsRes[resultIndex]._" +
            std::to_string((completeResIndex + 1)) + " = " +
            "(openingWindowsRes[idx].value._" + std::to_string((i + 1)) + "+" +
    ...
    if (m_numberOfKeyAttributes == 1) {
      ...
      for (auto i = 0; i < m_numberOfKeyAttributes; ++i) {
        s.append("                completeWindowsRes[resultIndex]._" +
                 std::to_string(i + 1) + " = partialRes[idx].key._" +
                 std::to_string(i) + ";\n");
      }
    }
    s.append("                /* Put value(s) */\n");
    ...
    s.append(
        ...
        "            for (int idx = start2; idx < end2; idx++) {\n"
        "                if (partialRes[idx].state != 1) /* Skip empty "
        "slot */\n"
        "                    continue;\n"
        "\n"
        "                /* Create a new hash table entry */\n"
        "                isFound = map1.get_index(partialRes[idx].key, "
        "posInRes); //isFound = "
        "map2.get_index(&openingWindowsResults[resultIndex], "
        "&buffer2[idx].key, posInRes);\n"
        //"                if (posInRes < 0 || isFound) {\n"
        //"                    printf(\"error: failed to insert new key in
        // intermediate hash table \\n\");\n" " exit(1);\n" " }\n"
        "                if (!isFound) {\n"
        "                    /* Mark occupancy */\n"
        "                    openingWindowsRes[posInRes + "
        "resultIndex].state = 1;\n"
        "                    /* Put timestamp */\n"
        "                    openingWindowsRes[posInRes + "
        "resultIndex].timestamp = partialRes[idx].timestamp;\n"
        "                    /* Put key and value(s) */\n"
        "                    openingWindowsRes[posInRes + resultIndex].key "
        "= partialRes[idx].key;\n"
        "                    openingWindowsRes[posInRes + "
        "resultIndex].value = partialRes[idx].value;\n"
        "                    openingWindowsRes[posInRes + "
        ...
        "                    /* Put timestamp */\n"
        "                    openingWindowsRes[posInRes + "
        "resultIndex].timestamp = std::max(openingWindowsRes[posInRes + "
        "resultIndex].timestamp, partialRes[idx].timestamp);\n"
        "                    /* Put key and value(s) */\n"
        "                    openingWindowsRes[posInRes + resultIndex].key "
        "= openingWindowsRes[posInRes + resultIndex].key;\n");
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\t\t\topeningWindowsRes[posInRes + resultIndex].value._" +
                 std::to_string((i + 1)) + " = " +
                 "(openingWindowsRes[posInRes + resultIndex].value._" +
                 std::to_string((i + 1)) + "+" + "partialRes[idx].value._" +
                 std::to_string((i + 1)) + "); // /" +
                 "(openingWindowsRes[posInRes + "
                 "resultIndex].counter+partialRes[idx].counter);\n");
      } else
    ...
    s.append(
        "                    /* Put count */\n"
        "                    openingWindowsRes[posInRes + resultIndex].counter "
        "= openingWindowsRes[posInRes + resultIndex].counter + "
        "partialRes[idx].counter;\n"
        ...
        "        }\n"
        "    }\n"
        "    // return the pointer required for appending or prepending the "
        "results\n"
        "    return (pack) ? resultIndex*sizeof(output_tuple_t) : "
        "(numOfWindows*mapSize)*sizeof(Bucket);\n"
        "}\n");
    s.append("\n");
    return s;
  }
  ...
  std::string getHashTableMergeWithPtrsString() {
    ...
    s.append(
        ...
        "                if (openingWindowsRes[wid][idx].state != 1) /* Skip "
        "empty slot */\n"
        "                    continue;\n"
        "                isFound = "
        "map2.get_index(openingWindowsRes[wid][idx].key, posInB2);\n"
        "                if (posInB2 < 0) {\n"
        "                    printf(\"error: open-adress hash table is full "
        "\\n\");\n"
        "                    exit(1);\n"
        ...
        "                    /* Put timestamp */\n"
        "                    completeWindowsRes[resultIndex].timestamp = "
        "openingWindowsRes[wid][idx].timestamp;\n"
        "                    /* Put key */\n");
    if (m_numberOfKeyAttributes == 1) {
      s.append(
          "                    completeWindowsRes[resultIndex]._1 = "
          "openingWindowsRes[wid][idx].key;\n");
    } else {
      for (auto i = 0; i < m_numberOfKeyAttributes; ++i) {
        s.append("                    completeWindowsRes[resultIndex]._" +
                 std::to_string(i + 1) +
    ...
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\t\t\tcompleteWindowsRes[resultIndex]._" +
                 std::to_string((completeResIndex + 1)) + " = " +
                 "openingWindowsRes[wid][idx].value._" +
                 std::to_string((i + 1)) +
    ...
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\t\t\tcompleteWindowsRes[resultIndex]._" +
                 ...
                 ")/" +
                 "(openingWindowsRes[wid][idx].counter+partialRes[wid2]["
                 "posInB2].counter);\n");
      } else
        s.append(
            "\t\t\t\t\tcompleteWindowsRes[resultIndex]._" +
    ...
    s.append(
        ...
        "*/\n"
        "            for (int idx = 0; idx < mapSize; idx++) {\n"
        "                if (partialRes[wid2][idx].state != 1) /* Skip empty "
        "slot */\n"
        "                    continue;                    \n"
        "                /* Copy tuple based on output schema */\n"
        "                /* Put timestamp */\n"
        "                completeWindowsRes[resultIndex].timestamp = "
        "partialRes[wid2][idx].timestamp;\n"
        "                /* Put key */\n");
    if (m_numberOfKeyAttributes == 1) {
      s.append(
          "                completeWindowsRes[resultIndex]._1 = "
          "partialRes[wid2][idx].key;\n");
    } else {
      for (auto i = 0; i < m_numberOfKeyAttributes; ++i) {
        s.append("                completeWindowsRes[resultIndex]._" +
                 std::to_string(i + 1) + " = partialRes[wid2][idx].key._" +
                 std::to_string(i) + ";\n");
      }
    }
    s.append("                /* Put value(s) */\n");
    ...
    s.append(
        ...
        "            /* Iterate over the second table. */\n"
        "            for (int idx = 0; idx < mapSize; idx++) {\n"
        "                if (partialRes[wid2][idx].state != 1) /* Skip "
        "empty slot */\n"
        "                    continue;\n"
        "\n"
        "                /* Create a new hash table entry */\n"
        "                isFound = "
        "map1.get_index(partialRes[wid2][idx].key, posInRes); //isFound = "
        "map2.get_index(&openingWindowsResults[resultIndex], "
        "&buffer2[idx].key, posInRes);\n"
        //"                if (posInRes < 0 || isFound) {\n"
        //"                    printf(\"error: failed to insert new key in
        ...
        "                    /* Put timestamp */\n"
        "                    openingWindowsRes[wid][posInRes].timestamp = "
        "partialRes[wid2][idx].timestamp;\n"
        "                    /* Put key and value(s) */\n"
        "                    openingWindowsRes[wid][posInRes].key = "
        "partialRes[wid2][idx].key;\n"
        "                    openingWindowsRes[wid][posInRes].value = "
        "partialRes[wid2][idx].value;\n"
        "                    openingWindowsRes[wid][posInRes].counter = "
        "partialRes[wid2][idx].counter;\n"
        "                } else {\n"
        "                    /* Mark occupancy */\n"
        "                    openingWindowsRes[wid][posInRes].state = 1;\n"
        "                    /* Put timestamp */\n"
        "                    openingWindowsRes[wid][posInRes].timestamp = "
        "std::max(openingWindowsRes[wid][posInRes].timestamp, "
        "partialRes[wid2][idx].timestamp);\n"
        "                    /* Put key and value(s) */\n"
        "                    openingWindowsRes[wid][posInRes].key = "
        "openingWindowsRes[wid][posInRes].key;\n");
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\t\t\topeningWindowsRes[wid][posInRes].value._" +
                 std::to_string((i + 1)) + " = " +
                 "(openingWindowsRes[wid][posInRes].value._" +
                 std::to_string((i + 1)) + "+" +
                 "partialRes[wid2][idx].value._" + std::to_string((i + 1)) +
                 "); // /" +
                 "(openingWindowsRes[wid][posInRes].counter+partialRes[wid2]["
                 "idx].counter);\n");
      } else
        s.append("\t\t\t\t\topeningWindowsRes[wid][posInRes].value._" +
                 std::to_string((i + 1)) + " = " +
  ...
  std::string getHashTableParallelMergeWithPtrsString() {
    ...
    if (!hasHavingPredicate())
      ...
      completeResPredicate = "resultIndex += " + getHavingExpr(false) + ";\n";
    s.append(
        "long aggregatePartials (char *completeBuffer, int completeWindow, int "
        "completeStartPos,\n"
        ...
        "    int currentWindowSpacing = completeStartPos + completeWindow * "
        "hashTableSpacing;\n"
        "\n"
        "    // Input and Output Buffers\n"
        "    Bucket **partialRes= (partialBuffer!=nullptr) ? (Bucket **) "
        "partialBuffer : nullptr;\n"
        "    Bucket *tempCompleteWindowsRes = (Bucket *) "
        "(completeBuffer+currentWindowSpacing);\n"
        "    output_tuple_t *completeWindowsRes = (output_tuple_t *) "
        "(completeBuffer+currentWindowSpacing); // the results here are "
        "packed\n"
        "\n"
        "    // Temp variables for the merging\n"
        "    bool isFound;\n"
        ...
        "        throw std::runtime_error (\"error: resize unbounded buffer: "
        "\" + std::to_string(currentWindowSpacing+hashTableSpacing) + \" - \" "
        "+ std::to_string(UNBOUNDED_SIZE) + \"\\n\");\n"
        "    }\n"
        "\n"
        "    if (startPos == -1) {\n"
        "        // memcpy the first opening buffer to the correct result "
        "slot\n"
        "        memcpy(completeBuffer+currentWindowSpacing, "
        "partialRes[partialWindow], mapSize * tupleSize * sizeof(char));\n"
        "        startPos = 0;\n"
        "        endPos = 0;\n"
        "        return 0;\n"
        "    }\n"
        ...
        "        /* Iterate over the second table. */\n"
        "        for (int idx = 0; idx < mapSize; idx++) {\n"
        "            if (partialRes[partialWindow][idx].state != 1) /* Skip "
        "empty slot */\n"
        "                continue;\n"
        "            isFound = "
        "leftMap.get_index(partialRes[partialWindow][idx].key, posInLeft);\n"
        "            if (posInLeft < 0) {\n"
        "                    printf(\"error: open-adress hash table is full "
        "\\n\");\n"
        "                    exit(1);\n"
        ...
        "                /* Put timestamp */\n"
        "                tempCompleteWindowsRes[posInLeft].timestamp = "
        "partialRes[partialWindow][idx].timestamp;\n"
        "                /* Put key and value(s) */\n"
        "                tempCompleteWindowsRes[posInLeft].key = "
        "partialRes[partialWindow][idx].key;\n"
        "                tempCompleteWindowsRes[posInLeft].value = "
        "partialRes[partialWindow][idx].value;\n"
        "                tempCompleteWindowsRes[posInLeft].counter = "
        "partialRes[partialWindow][idx].counter;\n"
        "            } else {\n"
        "                /* Mark occupancy */\n"
        "                tempCompleteWindowsRes[posInLeft].state = 1;\n"
        "                /* Put timestamp */\n"
        "                tempCompleteWindowsRes[posInLeft].timestamp = "
        "std::max(tempCompleteWindowsRes[posInLeft].timestamp, "
        "partialRes[partialWindow][idx].timestamp);\n"
        "                /* Put key and value(s) */\n"
        "                tempCompleteWindowsRes[posInLeft].key = "
        "tempCompleteWindowsRes[posInLeft].key;\n");
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\t\ttempCompleteWindowsRes[posInLeft].value._" +
                 std::to_string((i + 1)) + " = " +
                 "(tempCompleteWindowsRes[posInLeft].value._" +
                 std::to_string((i + 1)) + "+" +
                 "partialRes[partialWindow][idx].value._" +
                 std::to_string((i + 1)) + "); // /" +
                 "(tempCompleteWindowsRes[posInLeft].counter+partialRes["
                 "partialWindow][idx].counter);\n");
      } else
        s.append(
            "\t\t\t\ttempCompleteWindowsRes[posInLeft].value._" +
            std::to_string((i + 1)) + " = " +
    ...
    s.append(
        ...
        "    } else {\n"
        "        for (int idx = 0; idx < mapSize; idx++) {\n"
        "            if (tempCompleteWindowsRes[idx].state != 1) /* Skip empty "
        "slot */\n"
        "                continue;\n"
        "            /* Copy tuple based on output schema */\n"
        "            /* Put timestamp */\n"
        "            completeWindowsRes[resultIndex].timestamp = "
        "tempCompleteWindowsRes[idx].timestamp;\n"
        "            /* Put key */\n");
    if (m_numberOfKeyAttributes == 1) {
      s.append(
          "            completeWindowsRes[resultIndex]._1 = "
          "tempCompleteWindowsRes[idx].key;\n");
    } else {
      for (auto i = 0; i < m_numberOfKeyAttributes; ++i) {
        s.append("            completeWindowsRes[resultIndex]._" +
                 std::to_string(i + 1) +
    ...
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\tcompleteWindowsRes[resultIndex]._" +
                 std::to_string((completeResIndex + 1)) + " = " +
                 "tempCompleteWindowsRes[idx].value._" +
                 std::to_string((i + 1)) +
    ...
    s.append(addTabs(completeResPredicate, 3) +
             "            // Do I need padding here ???                \n"
             "        }\n"
             "\n"
             "        // set result positions\n"
             "        startPos = currentWindowSpacing;\n"
             "        endPos = currentWindowSpacing + "
             "sizeof(output_tuple_t)*resultIndex;\n"
             "    }\n"
             "    return 0;\n"
             "}\n");
    s.append("\n");
  ...
  std::string getSingleStaticDeclaration() {
    std::string s;
    s.append("static Aggregator aggrStructures[" +
             std::to_string(SystemConf::getInstance().WORKER_THREADS) +
             "];\n\n");
    return s;
  }
  ...
  std::string getSingleKeyMergeString() {
    ...
    s.append(
        ...
        "    // Input and Output Buffers\n"
        "    interm_tuple_t *openingWindowsRes= (interm_tuple_t *) "
        "openingBuffer;\n"
        "    interm_tuple_t *partialRes= (interm_tuple_t *) "
        "closingOrPendingBuffer;\n"
        "    output_tuple_t *completeWindowsRes = (output_tuple_t *) "
        "completeBuffer; // the results here are packed\n"
        "\n"
        "    // Temp variables for the merging\n"
        "    int resultIndex = (pack) ? "
        "completeBufferPtr/sizeof(output_tuple_t) : startingWindow;\n"
        "\n"
        "    int start1, end1, start2, end2;\n"
        "    for (int wid = startingWindow; wid < numOfWindows; ++wid) {\n"
        "        start1 = openingStartPointers[wid];\n"
        ...
        "            exit(1);\n"
        "        }\n"
        "        // merge values based on the number of aggregated values and "
        "their types!\n"
        "        if (pack) {\n"
        "            /* Copy tuple based on output schema */\n"
        "            /* Put timestamp */\n"
        "            completeWindowsRes[resultIndex].timestamp = "
        "std::max(openingWindowsRes[start1].timestamp, "
        "partialRes[start2].timestamp);\n"
        "            /* Put value(s) */\n");
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\tcompleteWindowsRes[resultIndex]._" +
                 std::to_string((i + 1)) + " = " +
                 "(openingWindowsRes[start1]._" + std::to_string((i + 1)) +
                 "+" + "partialRes[start2]._" + std::to_string((i + 1)) + ")/" +
                 "(openingWindowsRes[start1]._" +
                 std::to_string((m_outputSchema->numberOfAttributes() - 1)) +
                 "+" + "partialRes[start2]._" +
                 std::to_string((m_outputSchema->numberOfAttributes() - 1)) +
                 ");\n");
      } else
    ...
    s.append(
        ...
        "        } else {\n"
        "            openingWindowsRes[start1].timestamp = "
        "partialRes[start2].timestamp;\n"
        "            /* Put value(s) */\n");
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("\t\t\topeningWindowsRes[start1]._" + std::to_string((i + 1)) +
                 " = " + "openingWindowsRes[start1]._" +
                 std::to_string((i + 1)) + "+" + "partialRes[start2]._" +
                 std::to_string((i + 1)) + ";\n");
        s.append("\t\t\topeningWindowsRes[start1]._" +
                 std::to_string((m_outputSchema->numberOfAttributes() - 1)) +
                 " = " + "openingWindowsRes[start1]._" +
                 std::to_string((m_outputSchema->numberOfAttributes() - 1)) +
                 "+" + "partialRes[start2]._" +
    ...
    s.append(
        ...
        "results\n"
        "    return (pack) ? resultIndex*sizeof(output_tuple_t) : "
        "(numOfWindows)*sizeof(interm_tuple_t);\n"
        "}\n");
    s.append("\n");
    return s;
  }

  std::string getSelectionExpr() {
    std::string s;
    if (hasSelection()) {
      s.append("if ( ");
  ...
  std::string getHavingExpr(bool isCompute) {
    ...
    if (isCompute)
      s.replace(s.find(str), str.length(),
                "completeWindowsResults[completeWindowsPointer]");
    else
  ...
  std::string getProjectionExpr() {
    std::string s;
    if (hasProjection() && m_expressions != nullptr &&
        !m_expressions->empty()) {
      s.append("\toutput[pos].timestamp = data[bufferPtr].timestamp;\n");
      for (unsigned long i = 1; i < m_expressions->size(); ++i) {
        s.append("\toutput[pos]._" + std::to_string(i) + " = " +
                 (*m_expressions)[i]->toSExprForCodeGen() + ";\n");
      }
    }
    return s;
  }
  ...
  std::string getAggregationVarsOutputBuffers() {
    std::string s;
    s.append(
        "interm_tuple_t *openingWindowsResults = (interm_tuple_t *) "
        "openingWindowsBuffer; // the results here are in the\n"
        "interm_tuple_t *closingWindowsResults = (interm_tuple_t *) "
        "closingWindowsBuffer; // form of the hashtable\n"
        "interm_tuple_t *pendingWindowsResults = (interm_tuple_t *) "
        "pendingWindowsBuffer;\n");
    return s;
  }
  ...
  std::string getAggregationVarsComputation(bool isRangeBased) {
    ...
    if (hasAggregation()) {
      for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
        ...
        switch ((*m_aggregationTypes)[i]) {
          case SUM:
            s.append("aggrs._" + std::to_string(i + 1) + " += " + input +
                     ";\n");
            break;
          case AVG:
            s.append("aggrs._" + std::to_string(i + 1) + " += " + input +
          ...
          case MIN:
            s.append("aggrs._" + std::to_string(i + 1) + " = " +
                     getCombineFunction(MIN, "aggrs._" + std::to_string(i + 1),
                                        input) +
                     ";\n");
            break;
          case MAX:
            s.append("aggrs._" + std::to_string(i + 1) + " = " +
                     getCombineFunction(MAX, "aggrs._" + std::to_string(i + 1),
                                        input) +
                     ";\n");
            break;
          default:
  ...
  std::string getWriteIntermediateResults(int bufferType) {
    ...
    if ((int)m_aggregationTypes->size() > m_outputSchema->numberOfAttributes())
      throw std::runtime_error(
          "error: the number of aggregation types should be <= to the "
          "attributes of the output schema");
    std::string s;
    std::string buffer;
    if (bufferType == 0)
    ...
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("" + buffer + "._" + std::to_string((i + 1)) + " = aggrs._" +
                 std::to_string((i + 1)) + ";\n");
        s.append("" + buffer + "._" +
                 std::to_string((m_outputSchema->numberOfAttributes() - 1)) +
                 " = aggrs._c" + std::to_string((i + 1)) + ";\n");
      } else
        s.append("" + buffer + "._" + std::to_string((i + 1)) + " = aggrs._" +
                 std::to_string((i + 1)) + ";\n");
    }
  ...
  std::string getWriteCompleteResults() {
    ...
    std::string s;
    s.append(
        "completeWindowsResults[completeWindowsPointer].timestamp = "
        "prevCompletePane * paneSize;\n");  // data[bufferPtr-1].timestamp;\n");
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("completeWindowsResults[completeWindowsPointer]._" +
                 std::to_string((i + 1)) + " = aggrs._" +
                 std::to_string((i + 1)) + "/aggrs._c" +
                 std::to_string((i + 1)) + ";\n");
      } else
        s.append("completeWindowsResults[completeWindowsPointer]._" +
                 std::to_string((i + 1)) + " = aggrs._" +
    ...
    if (!hasHavingPredicate())
      ...
      completeResPredicate =
          "completeWindowsPointer += " + getHavingExpr(true) + ";\n";
  ...
  std::string getAggregationVarsResultPointers() {
    std::string s;
    s.append(
        "pointersAndCounts[0] = openingWindowsPointer * "
        "sizeof(interm_tuple_t);\n"
        "pointersAndCounts[1] = closingWindowsPointer * "
        "sizeof(interm_tuple_t);\n"
        "pointersAndCounts[2] = pendingWindowsPointer * "
        "sizeof(interm_tuple_t);\n"
        "pointersAndCounts[3] = completeWindowsPointer * "
        "sizeof(output_tuple_t);\n");
    return s;
  }
  ...
  std::string getGroupByOutputBuffers() {
    ...
    if (!m_usePtrs) {
      s.append(
          "Bucket *openingWindowsResults = (Bucket *) openingWindowsBuffer; // "
          "the results here are in the\n"
          "Bucket *closingWindowsResults = (Bucket *) closingWindowsBuffer; // "
          "form of the hashtable\n"
      ...
      s.append(
          ...
          "Bucket **closingWindowsResults = (Bucket **) closingWindowsBuffer; "
          "// form of the hashtable\n"
          "Bucket **pendingWindowsResults = (Bucket **) "
          "pendingWindowsBuffer;\n");
    }
  ...
  std::string getGroupByComputation(bool isRangeBased) {
    ...
    if (hasAggregation()) {
      for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
        auto col = (*m_aggregationAttributes)[i]->getColumn();
        if (col == 0)
          s.append("curVal._" + std::to_string(i + 1) +
                   " = data[bufferPtr].timestamp;\n");
        else
          s.append("curVal._" + std::to_string(i + 1) + " = data[bufferPtr]._" +
                   std::to_string(col) + ";\n");
      }
      ...
      if (m_numberOfKeyAttributes == 1) {
        std::string col;
        if (auto cRef =
                dynamic_cast<ColumnReference *>((*m_groupByAttributes)[0])) {
          if (cRef->getColumn() == 0)
            col = "timestamp";
          else
            col = "_" + std::to_string(cRef->getColumn());
          s.append("aggrStructures[pid].insert_or_modify(data[bufferPtr]." +
                   col + ", curVal, data[bufferPtr].timestamp);\n");
        } else {
          s.append("aggrStructures[pid].insert_or_modify(" +
                   (*m_groupByAttributes)[0]->toSExprForCodeGen() +
        ...
        for (int i = 0; i < m_numberOfKeyAttributes; ++i) {
          if (auto cRef =
                  dynamic_cast<ColumnReference *>((*m_groupByAttributes)[i])) {
            if (cRef->getColumn() == 0)
              col = "timestamp";
            else
              col = "_" + std::to_string(cRef->getColumn());
        ...
        s.append("};\n");
        s.append(
            "aggrStructures[pid].insert_or_modify(tempKey, curVal, "
            "data[bufferPtr].timestamp);\n");
      }
    }
    if (hasProjection()) {
  ...
  std::string getWriteIntermediateResultsGroupBy(int bufferType) {
    ...
    if (!m_usePtrs) {
      if (bufferType == 0)
        s.append(
            "std::memcpy(closingWindowsResults + closingWindowsPointer, "
            "aggrStructures[pid].getBuckets(), mapSize * sizeof(Bucket));\n");
      else if (bufferType == 1)
        s.append(
            "std::memcpy(openingWindowsResults + openingWindowsPointer, "
            "aggrStructures[pid].getBuckets(), mapSize * sizeof(Bucket));\n");
      else
        s.append(
            "std::memcpy(pendingWindowsResults + pendingWindowsPointer, "
            "aggrStructures[pid].getBuckets(), mapSize * sizeof(Bucket));\n");
    } else {
      if (bufferType == 0)
        s.append(
            "std::memcpy(closingWindowsResults[numberOfClosingWindows], "
            "aggrStructures[pid].getBuckets(), mapSize * sizeof(Bucket));\n");
      else if (bufferType == 1)
        s.append(
            "std::memcpy(openingWindowsResults[numberOfOpeningWindows], "
            "aggrStructures[pid].getBuckets(), mapSize * sizeof(Bucket));\n");
      else
        s.append(
            "std::memcpy(pendingWindowsResults[0], "
            "aggrStructures[pid].getBuckets(), mapSize * sizeof(Bucket));\n");
    }
  ...
  std::string getWriteCompleteResultsGroupBy() {
    ...
    if (m_numberOfKeyAttributes == 1) {
      ...
      for (auto i = 0; i < m_numberOfKeyAttributes; ++i) {
        s.append("completeWindowsResults[completeWindowsPointer]._" +
                 std::to_string((i + 1)) +
                 " = aggrStructures[pid].getBuckets()[i].key._" +
                 std::to_string(i) + ";\n");
      }
    }
    ...
    for (unsigned long i = 0; i < m_aggregationTypes->size(); ++i) {
      if ((*m_aggregationTypes)[i] == AVG) {
        s.append("completeWindowsResults[completeWindowsPointer]._" +
                 std::to_string((m_numberOfKeyAttributes + 1)) +
                 " = aggrStructures[pid].getBuckets()[i].value._" +
                 std::to_string((i + 1)) +
    ...
    s.append(
        "completeWindowsPointer += aggrStructures[pid].getBuckets()[i].state" +
        completeResPredicate + ";\n");
    return s;
  }
  ...
  std::string getTumblingWindowRows(
      std::string outputBuffers, std::string initialiseAggrs,
      std::string computeAggrs, std::string resetAggrs,
      std::string openingWindows, std::string closingWindows,
      std::string pendingWindows, std::string completeWindows,
      std::string resultPointers) {
    std::string s;
    std::string ptr;
    if (hasGroupBy() && m_usePtrs) {
    ...
    s.append(
        "void processData (int pid, char *inputBuffer, size_t inputBufferSize, "
        "long startPointer, long endPointer, long timestampFromPrevBatch,\n"
        ...
        "*closingStartPointers, int *pendingStartPointers, int "
        "*completeStartPointers,\n"
        //"                  long *openingWindowIds, long *closingWindowIds,
        // long *pendingWindowIds, long *completeWindowIds,\n"
        "                  long streamStartPointer, int *pointersAndCounts, "
        "char *staticBuffer) {"
        "    // Input Buffer\n"
        "    input_tuple_t *data= (input_tuple_t *) inputBuffer;\n"
        "\n"
        ...
        "    output_tuple_t *completeWindowsResults = (output_tuple_t *) "
        "completeWindowsBuffer; // the results here are packed\n"
        "\n"
        ...
        "    long prevClosePane, currPane, prevCompletePane, prevOpenPane, "
        "startPane;\n"
        ...
        "    prevClosePane = prevOpenPane = (streamPtr / paneSize) - "
        "panesPerSlide;\n"
        ...
        "\n"
        "\n"
        "    // The beginning of the stream. Check if we have at least one "
        "complete window.\n"
        "    if (streamPtr == 0) {\n"
        "        // check for opening windows until finding the first "
        "complete\n"
        ...
        "        }\n"
        "    }\n"
        "        // Check for closing and opening windows, until we have a "
        "complete window.\n"
        "    else {\n"
        "        while (bufferPtr < endPointer) {\n"
        "            currPane = streamPtr / paneSize;\n"
        "            if (currPane - prevOpenPane == panesPerSlide) { // new "
        "slide and possible opening windows\n"
        "                prevOpenPane += panesPerSlide;\n"
        "                windowStartPointers[currentSlide++] = bufferPtr;\n"
        "            }\n"
        "            if (hasComplete && currPane - prevCompletePane == "
        "panesPerWindow) { // complete window\n"
        "                windowEndPointers[currentWindow++] = bufferPtr;\n"
        "                numberOfCompleteWindows++;\n"
        "                break;\n"
        "            }\n"
        "            if (/*prevClosePane >= panesPerWindow &&*/ prevClosePane "
        "<= startPane && currPane - prevClosePane == panesPerWindow) { // "
        "closing window\n"
        ...
        "                closingStartPointers[numberOfClosingWindows] = "
        "closingWindowsPointer;\n"
        //"                closingWindowIds[numberOfClosingWindows-1] =
        // prevClosePane - panesPerSlide;\n"
        "                // reset values\n" +
        ...
        "        if ((numberOfClosingWindows == 0 || windowSize!=windowSlide) "
        "&& numberOfCompleteWindows == 0) {\n" +
        //"            currPane = streamPtr / paneSize;\n"
        //"            if (/*prevClosePane >= panesPerWindow &&*/ prevClosePane
        //<= startPane && currPane - prevClosePane == panesPerWindow) { //
        // closing window\n" "                // write result to the closing
        // windows\n" + addTabs(closingWindows, 5) + " closingWindowsPointer +=
        // mapSize;\n" "                numberOfClosingWindows++;\n" "
        // closingStartPointers[numberOfClosingWindows] =
        // closingWindowsPointer;\n" "                // reset values\n" +
        // addTabs(resetAggrs, 4) +
        ...
        "            pendingStartPointers[numberOfPendingWindows] = "
        ...
        "    }\n"
        "\n"
        "    if (numberOfCompleteWindows == 0 && (streamStartPointer == 0 || "
        "currentSlide >= 1)) { // We only have one opening window so far...\n"
        "        // write results\n" +
        addTabs(openingWindows, 2) +
        "        openingWindowsPointer += mapSize;\n"
        "        numberOfOpeningWindows++;\n"
        "        openingStartPointers[numberOfOpeningWindows] = "
        "openingWindowsPointer;\n"
        //"        openingWindowIds[numberOfOpeningWindows-1] = prevOpenPane;\n"
        "\n"
        "    } else if (numberOfCompleteWindows > 0) {\n"
        "        // write results and pack them for the first complete window "
        "in the batch\n"
        ...
        "        // write in the correct slot, as the value has already been "
        "incremented!\n"
        "        completeStartPointers[numberOfCompleteWindows] = "
        "completeWindowsPointer;\n"
        //"        completeStartWindowIds[numberOfCompleteWindows-1] =
        // prevCompleteWindow;\n"
        "        // filter, project + aggregate here\n" +
        addTabs(computeAggrs, 2) +
        "\n"
        "        bufferPtr = windowEndPointers[0];\n"
        "        prevOpenPane = (windowStartPointers[currentSlide-1] +diff) / "
        "paneSize;\n"
        "        int idx = 1;\n"
        "        prevCompletePane = (windowStartPointers[idx++]+diff) / "
        "paneSize;\n"
        "        int removalIndex = currentWindow; //(startingFromPane) ? "
        "currentWindow : currentWindow + 1;\n"
        "        bool foundComplete = false;\n"
        "        while (bufferPtr < endPointer) {\n"
        "            // add elements from the next slide\n"
        "            bufferPtr = windowEndPointers[currentWindow - 1] + 1; // "
        "take the next position, as we have already computed this value\n"
        "            foundComplete = false;\n"
        "            while (true) {\n"
        ...
        "                    //prevPane = currPane;\n"
        "                    prevCompletePane = "
        "(windowStartPointers[idx++]+diff) / paneSize;\n"
        "\n"
        "                    windowEndPointers[currentWindow++] = bufferPtr;\n"
        ...
        "                    completeStartPointers[numberOfCompleteWindows] = "
        "completeWindowsPointer;\n"
        //"                    completeWindowIds[numberOfCompleteWindows-1] =
        // prevCompletePane - 1;\n"
        "                    foundComplete = true;\n"
        "                    // reset values\n" +
        ...
        "        }\n"
        "\n"
        "        if (!foundComplete) {  // we have reached the first open "
        "window after all the complete ones\n"
        "            // write the first open window if we have already "
        "computed the result\n" +
        ...
        "            openingStartPointers[numberOfOpeningWindows] = "
        "openingWindowsPointer;\n"
        //"            openingWindowIds[numberOfOpeningWindows-1] =
        // prevCompletePane++;\n"
        "        }\n"
        "    }\n"
        "\n" +
  ...
  std::string getSlidingWindowRows(
      std::string outputBuffers, std::string initialiseAggrs,
      std::string computeAggrs, std::string insertAggrs, std::string evictAggrs,
      std::string resetAggrs, std::string setValues, std::string openingWindows,
      std::string closingWindows, std::string pendingWindows,
      std::string completeWindows, std::string resultPointers) {
    std::string s;
    std::string ptr;
    if (hasGroupBy() && m_usePtrs) {
      ptr.append("*");
    ...
    s.append(
        ...
        "                prevOpenPane += panesPerSlide;\n"
        "                windowStartPointers[currentSlide++] = bufferPtr; // "
        "TODO: maybe store bPtr*tupleSize\n"
        "                if (bufferPtr!=0) {\n" +
        addTabs(insertAggrs, 5) + addTabs(resetAggrs, 5) +
        ...
        "                prevPane = currPane;\n"
        "                if (numOfPartials==BUCKET_SIZE) // remove the extra "
        "values so that we have the first complete window\n" +
        addTabs(evictAggrs, 5) + addTabs(insertAggrs, 4) +
        addTabs(resetAggrs, 4) +
        ...
        "                break;\n"
        "            }\n"
        "            if (prevClosePane <= startPane && currPane - "
        "prevClosePane == panesPerWindow) { // closing window\n"
        "                // write result to the closing windows\n" +
        //"                aggrs =
        // aggrStructures[pid].queryIntermediate(PARENTS_SIZE-2);\n" +
        ...
        "                if (currPane-prevPane==1) {\n"
        "                    prevPane = currPane;\n" +
        addTabs(insertAggrs, 4) + addTabs(resetAggrs, 4) +
        //"                    aggrStructures[pid].insert(aggrs);\n"
        //"                    aggrs.reset();\n"
        "                }\n"
        "                if (currPane - prevOpenPane == panesPerSlide) {\n"
        ...
        //"            openingWindowIds[numberOfOpeningWindows-1] =
        // prevCompletePane++;\n"
        "        } else { // otherwise remove the respective tuples for the "
        ...
        "        currentWindow++;\n"
        "        if (tempStartPos == tempEndPos || tempEndPos==endPointer) "
        "continue;\n" +
  ...
  std::string getFillEmptyTumblingWindows(std::string resetAggrs,
    ...
    if (hasGroupBy()) {
      ...
      closingWindows.replace(closingWindows.find(str), str.length(),
                             "prevPane * paneSize;");
      completeWindows.replace(completeWindows.find(str), str.length(),
                              "prevPane * paneSize;");
      singleKey = "node aggrs;\n";
    }
    s.append(
        "void fillEmptyWindows (int pid, int phase, int numOfSlices, int "
        "numOfOpening, int numOfClosing,\n"
        "                          long *windowStartPointers, long "
        "*windowEndPointers,\n"
        "                          int &numberOfOpeningWindows, int "
        "&numberOfClosingWindows, int &numberOfCompleteWindows,\n"
        "                          int &currentSlide, int &currentWindow,\n"
        "                          int &completeWindowsPointer, output_tuple_t "
        "*completeWindowsResults, int *completeStartPointers,\n"
        "                          int &closingWindowsPointer, " +
        closingDef + " *" + ptr +
        "closingWindowsResults, int *closingStartPointers,\n"
        //"                           long *openingWindowIds, long
        //*closingWindowIds, long *pendingWindowIds, long *completeWindowIds,\n"
        "                          long &prevCompletePane, long "
        "&prevClosePane, long &prevOpenPane, long &currPane) {\n"
        "\n"
        ...
        "        // add more complete\n"
        "    } else if (phase == 2) {\n"
        "        // case 2 - opening and complete at the beginning of "
        "tumbling\n"
        "        auto prevPane = prevOpenPane + 1;\n"
        "        for (int ptr = 0; ptr < numOfSlices; ++ptr) {\n"
        ...
        "//data[(windowStartPointers[idx++])].timestamp / paneSize;\n"
        ...
        "            if (prevPane - prevClosePane == panesPerWindow) { // "
        "closing window\n"
        "                // write result to the closing windows\n" +
        addTabs(closingWindows, 4) +
        "                prevClosePane += panesPerSlide;\n"
        ...
        "            // fillBuckets with empty;\n"
        "            if (prevPane - prevOpenPane == panesPerSlide) { // new "
        "slide and possible opening windows\n"
        "                prevOpenPane += panesPerSlide;\n"
        "                windowStartPointers[currentSlide++] = -1;\n"
        "            }\n"
        ...
        "            // complete windows\n"
        "            if (prevPane - prevCompletePane == panesPerWindow) {\n"
        "                prevCompletePane += panesPerWindow; "
        "//data[(windowStartPointers[idx++])].timestamp / paneSize;\n"
        "                windowEndPointers[currentWindow++] = -1;\n"
        "                // write and pack the complete window result\n" +
        "                for (int i = 0; i < mapSize; i++) {\n" +
  ...
  std::string getTumblingWindowRange(
      std::string outputBuffers, std::string initialiseAggrs,
      std::string computeAggrs, std::string resetAggrs,
      std::string openingWindows, std::string closingWindows,
      std::string pendingWindows, std::string completeWindows,
      std::string resultPointers, std::string filter) {
    std::string s;
    std::string ptr;
    if (hasGroupBy() && m_usePtrs) {
      ptr.append("*");
...
//                "    timestampFromPrevBatch = (bufferPtr != 0) ?
//                data[bufferPtr - 1].timestamp / paneSize :\n" "
//                data[bufferSize / sizeof(input_tuple_t) - 1].timestamp /
//                paneSize;"
#endif
        "    long prevPane =  timestampFromPrevBatch / paneSize;\n"
        "\n"
        "    startPane = (data[bufferPtr].timestamp / paneSize);\n"
        "    prevClosePane = prevOpenPane = (data[bufferPtr].timestamp / "
        "paneSize) - panesPerSlide;\n"
        ...
        "            prevCompletePane += panesPerSlide;\n"
        "        //while (data[bufferPtr].timestamp-tmpPtr + windowSlide <= "
        "windowSize) {\n"
        "        //    tmpPtr -= windowSlide;\n"
        "        //}\n"
        "        //prevClosePane = tmpPtr / paneSize;\n"
        ...
        "\n"
        "    bool completeStartsFromPane = startPane==prevCompletePane;\n"
        "    bool hasComplete = ((data[endPointer-1].timestamp - "
        "data[startPointer].timestamp) >= windowSize);\n"
        "\n"
        "    // The beginning of the stream. Check if we have at least one "
        "complete window.\n"
        "    if (streamPtr == 0) {\n"
    ...
    s.append(
        ...
        "                break;\n"
        "            }\n"
        "            if (currPane - prevOpenPane > panesPerSlide || currPane - "
        "prevCompletePane > panesPerWindow) {\n"
        "                // fill bubbles\n"
        "                int numOfSlices, numOfComplete, numOfOpening, phase;\n"
        "                phase = 1;\n"
        "                //if (currPane - prevOpenPane > panesPerSlide) {\n"
        "                    numOfSlices = currPane - prevOpenPane; // - "
        "panesPerSlide -1;\n"
        "                    numOfComplete = "
        "currPane+numOfSlices-panesPerWindow;\n"
        "                //} else {\n"
        "                //    numOfSlices = currPane - prevCompletePane - "
        "panesPerWindow -1;\n"
        "                //    numOfComplete = "
        "currPane+numOfSlices-panesPerWindow;\n"
        "                //}\n"
        "                if (numOfComplete > 0) {\n"
        "                    numOfOpening =  prevCompletePane + panesPerWindow "
        "- prevOpenPane;\n"
        "                    //phase = 2;\n"
        "                    hasAddedComplete = true;\n"
        "                }\n"
        "                fillEmptyWindows(pid, phase, numOfSlices, "
        "numOfOpening, 0, windowStartPointers,\n"
        "                        windowEndPointers, numberOfOpeningWindows, "
        "numberOfClosingWindows, numberOfCompleteWindows,\n"
        "                                    currentSlide, currentWindow,\n"
        "                                    completeWindowsPointer, "
        "completeWindowsResults, completeStartPointers,\n"
        "                                    closingWindowsPointer, "
        "closingWindowsResults, closingStartPointers,\n"
        //"                                    openingWindowIds,
        // closingWindowIds, pendingWindowIds, completeWindowIds,\n"
        "                                    prevCompletePane, prevClosePane, "
        "prevOpenPane, currPane);\n"
        "            }\n"
        "            // project + aggregate here\n" +
        addTabs(computeAggrs, 3));
    if (!filter.empty()) s.append("\t\t\t};\n");
    s.append(
        "            streamPtr += step;\n"
        "            bufferPtr += step;\n"
        "        }\n"
    ...
    s.append(
        ...
        "            }\n"
        "            if (currPane - prevOpenPane > panesPerSlide || currPane - "
        "prevClosePane > panesPerWindow || currPane - prevCompletePane > "
        "panesPerWindow) {\n"
        "                // fill bubbles\n"
        "                int numOfSlices, numOfComplete, numOfOpening, "
        "numOfClosing,  phase;\n"
        "                numOfClosing = 0;\n"
        "                phase = 3;\n"
        "                if (currPane - prevOpenPane > panesPerSlide) {\n"
        "                    numOfSlices = currPane - prevPane; //prevOpenPane "
        "- panesPerSlide -1;\n"
        "                    numOfComplete = "
        "prevPane+numOfSlices-(panesPerWindow+prevCompletePane);\n"
        "                } else if (currPane - prevClosePane > panesPerWindow) "
        "{\n"
        "                    numOfSlices = currPane - prevClosePane - "
        "panesPerWindow -1;\n"
        ...
        "                }\n"
        "                fillEmptyWindows(pid, phase, numOfSlices, "
        "numOfOpening, numOfClosing, windowStartPointers,\n"
        "                                    windowEndPointers, "
        "numberOfOpeningWindows, numberOfClosingWindows, "
        "numberOfCompleteWindows,\n"
        "                                    currentSlide, currentWindow,\n"
        "                                    completeWindowsPointer, "
        "completeWindowsResults, completeStartPointers,\n"
    ...
    s.append(addTabs(computeAggrs, 2));
    if (!filter.empty()) s.append("\t\t}\n");
    s.append(
        "\n"
        "        bufferPtr = windowEndPointers[0];\n"
        "        //prevOpenPane = "
        "(data[windowStartPointers[currentSlide-1]].timestamp) / paneSize;\n"
        "        int idx = 1;\n"
        "        prevCompletePane += panesPerSlide; "
        "//(data[windowStartPointers[idx++]].timestamp) / paneSize;\n"
        "        int removalIndex = currentWindow; //(startingFromPane) ? "
    ...
    s.append(
        ...
        "                }\n"
        "                if (currPane - prevOpenPane > panesPerSlide || "
        "currPane - prevCompletePane > panesPerWindow) {\n"
        "                    // fill bubbles\n"
        "                    int numOfSlices, numOfComplete, numOfOpening, "
        "phase;\n"
        "                    phase = 2;\n"
        "                    //if (currPane - prevOpenPane > panesPerSlide) {\n"
        "                        numOfSlices = currPane - prevPane; "
        "//prevOpenPane - panesPerSlide -1;\n"
        "                        numOfComplete = "
        "prevPane+numOfSlices-(panesPerWindow+prevCompletePane);\n"
        "                    //} else {\n"
        "                    //    numOfSlices = currPane - prevCompletePane - "
        ...
        "                    if (numOfComplete > 0) {\n"
        "\n"
        "                        numOfOpening =  prevCompletePane + "
        "panesPerWindow - prevOpenPane;\n"
        "                        foundComplete = true;\n"
        "                    }\n"
        "                    fillEmptyWindows(pid, phase, numOfSlices, "
        "numOfOpening, 0, windowStartPointers,\n"
        ...
        // closingWindowIds, pendingWindowIds, completeWindowIds,\n"
        "                                        prevCompletePane, "
        "prevClosePane, prevOpenPane, currPane);\n"
        "                }\n"
        "                if (bufferPtr >= endPointer) {\n"
        "                    break;\n"
        "                }\n"
  ...
  std::string getFillEmptySlidingWindows(std::string insertAggrs,
    ...
    if (hasGroupBy()) {
      ...
      closingWindows.replace(closingWindows.find(str1), str1.length(),
                             "prevPane * paneSize;");
      completeWindows.replace(completeWindows.find(str2), str2.length(),
                              "prevPane");
      singleKey = "node aggrs;\n";
    }
    s.append(
        ...
        "                          int &numberOfOpeningWindows, int "
        "&numberOfClosingWindows, int &numberOfCompleteWindows,\n"
        "                          long bufferPtr, int &currentSlide, int "
        "&currentWindow,\n"
        "                          int &completeWindowsPointer, output_tuple_t "
        "*completeWindowsResults, int *completeStartPointers,\n"
        "                          int &closingWindowsPointer, " +
        ...
        addTabs(insertAggrs, 1) + addTabs(resetAggrs, 1) +
        "    if (phase == 1) {\n"
        "        // case 1 -- opening at the beginning\n"
        "        auto prevPane = prevOpenPane + 1;\n"
        "        int ptr;\n"
        ...
        "            // write results and pack them for the first complete "
        "window in the batch\n" +
        addTabs(setValues, 3) +
        "            for (int i = 0; i < mapSize; i++) {\n" +
        ...
        "            // reset values\n" +
        addTabs(resetAggrs, 3) +
        "            // write in the correct slot, as the value has already "
        "been incremented!\n"
        "            completeStartPointers[numberOfCompleteWindows] = "
        "completeWindowsPointer;\n"
        "            prevCompletePane += panesPerSlide;\n"
  ...
  std::string getSlidingWindowRange(
      std::string outputBuffers, std::string initialiseAggrs,
      std::string computeAggrs, std::string insertAggrs, std::string evictAggrs,
      std::string resetAggrs, std::string setValues, std::string openingWindows,
      std::string closingWindows, std::string pendingWindows,
...
//                paneSize;"
        ...
        "    if (streamStartPointer==0)\n"
        "        prevBatchPane = (data[bufferPtr].timestamp / paneSize) - "
        "panesPerSlide;\n"
        "    startPane = (data[bufferPtr].timestamp / paneSize);\n"
        "    prevClosePane = prevOpenPane = (data[bufferPtr].timestamp / "
        "paneSize) - panesPerSlide;\n"
    ...
    s.append(
        ...
        "                    numOfComplete = prevOpenPane + numOfSlices - "
        "panesPerWindow;\n"
        "                if (numOfComplete > 0) {\n"
    ...
    s.append(
        ...
        "                        numOfSlices = currPane - prevOpenPane; // - "
        "panesPerSlide -1;\n"
        "                        numOfComplete = prevPane + numOfSlices - "
        "(panesPerWindow + prevCompletePane);//numOfComplete = "
        "currPane+numOfSlices-panesPerWindow;\n"
        "                        if (numOfSlices == 0)\n"
        "                            prevOpenPane = currPane;\n"
        "                    } else if (currPane - prevClosePane >= "
        "panesPerWindow) {\n"
        "                        numOfSlices = currPane - prevClosePane; // - "
        "panesPerWindow -1;\n"
        ...
        "                        numOfSlices = currPane - prevCompletePane - "
        "panesPerWindow -1;\n"
        "                        numOfComplete = currPane + numOfSlices - "
        "panesPerWindow;\n"
        "                    }\n"
        "                    if (numOfComplete > 0) {\n"
        "                        hasAddedComplete = true;\n"
    ...
    s.append(
        ...
        "//prevOpenPane - panesPerSlide -1;\n"
        "                        numOfComplete = prevPane + numOfSlices - "
        "(panesPerWindow + prevCompletePane);\n"
        "                    //} else {\n"
        "                    //    numOfSlices = currPane - prevCompletePane - "
        "panesPerWindow -1;\n"
        "                    //    numOfComplete = "
    ...
    s.append(
        ...
        "        currentWindow = "
        "currentSlide-panesPerWindow+1;//+numberOfOpeningWindows;\n"
        "    if (numberOfCompleteWindows!=0 && "
        "currentSlide-currentWindow<panesPerWindow-1) {\n"
        ...
        "        tempEndPos = windowStartPointers[currentWindow + 1];\n"
        "        currentWindow++;\n"
        "        if ((tempStartPos == tempEndPos && tempStartPos!=-1) || "
        "((tempEndPos==0 /*|| tempEndPos==endPointer*/) /*&& \n"
    ...
    if (hasGroupBy())
      s.append(
          "        closingWindowsPointer -= (numberOfClosingWindows - "
          "panesPerWindow + 1)*mapSize;\n");
    else
      s.append(
          "        closingWindowsPointer -= (numberOfClosingWindows - "
          "panesPerWindow + 1);\n");
