...
void compressInput(int pid, char *input, int start, int end, char *output, int &writePos, int length, bool &clear, long latency = -1) {
  ...
  for (size_t idx = 0; idx < n; idx++) {
    ...
    Key temp{data[idx]._2, data[idx]._3, data[idx]._4};
    auto fVal_2 = (*dcomp)[pid]->compress(temp);
    auto sVal_2 = fVal_2;
    if (idx < n - 1) {
      Key temp2{data[idx+1]._2, data[idx+1]._3, data[idx+1]._4};
      sVal_2 = (*dcomp)[pid]->compress(temp2);
      if (sVal_2 == fVal_2) {
        count_2++;
      } else {
    ...
    auto fVal_3 = data[idx]._1;
    auto sVal_3 = fVal_3;
    if (idx < n - 1 &&
        fVal_3 == (sVal_3 = data[idx + 1]._1)) {
      count_3++;
    } else {
...
class LRB3 : public LinearRoadBenchmark {
 private:
  void createApplication() override {
    ...
    auto config3 =
        new QueryConfig(8 * SystemConf::getInstance().CIRCULAR_BUFFER_SIZE,
                        SystemConf::getInstance().BATCH_SIZE,
                        SystemConf::getInstance().BUNDLE_SIZE, 2 * 1024, 1024);
    ...
    aggregationAttributes_[0] = new ColumnReference(1, BasicType::Float);
    ...
    genCode_->setup(config3);
    OperatorCode *cpuCode_ = genCode_;

    // Define an ft-operator
    auto queryOperator_ = new QueryOperator(*cpuCode_, true);
    ...
    std::vector<std::shared_ptr<Query>> queries(2);
    ...
    queries[1] = std::make_shared<Query>(1, operators_, *window_, schema_, m_timestampReference,
                                         true,
                                         false,
                                         true,
                                         false, //useParallelMerge,
                                         0, false, config3);
    queries[0]->connectTo(queries[1].get());

    if (SystemConf::getInstance().CHECKPOINT_COMPRESSION) {
      queries[0]->getBuffer()->setCompressionFP(LRB3Compress::compressInput_);
      queries[1]->getBuffer()->setCompressionFP(LRB3Compress_::compressInput);
      queries[0]->getBuffer()->setDecompressionFP(LRB3Compress::decompressInput);
      queries[1]->getBuffer()->setDecompressionFP(LRB3Compress_::decompressInput);
    }
    ...
    if (SystemConf::getInstance().CHECKPOINT_COMPRESSION && (SystemConf::getInstance().CHECKPOINT_ON || persistInput)) {
      LRB3Compress::metadata = new std::vector<std::string, tbb::cache_aligned_allocator<std::string>>(SystemConf::getInstance().WORKER_THREADS, "");
      LRB3Compress_::metadata = new std::vector<std::string, tbb::cache_aligned_allocator<std::string>>(SystemConf::getInstance().WORKER_THREADS, "");
      LRB3Compress::dcomp = new std::vector<std::unique_ptr<
          DictionaryCompressor<LRB3Compress::Key, uint16_t, LRB3Compress::hash,
                               LRB3Compress::Eq>>>();
      for (int w = 0; w < SystemConf::getInstance().WORKER_THREADS; ++w) {
        LRB3Compress::dcomp->emplace_back(
            std::make_unique<DictionaryCompressor<LRB3Compress::Key, uint16_t,
                                                  LRB3Compress::hash, LRB3Compress::Eq>>(
                SystemConf::getInstance().HASH_TABLE_SIZE));
      }

      LRB3Compress_::dcomp = new std::vector<std::unique_ptr<
          DictionaryCompressor<LRB3Compress_::Key, uint16_t, LRB3Compress_::hash,
                               LRB3Compress_::Eq>>>();
      for (int w = 0; w < SystemConf::getInstance().WORKER_THREADS; ++w) {
        LRB3Compress_::dcomp->emplace_back(
            std::make_unique<DictionaryCompressor<LRB3Compress_::Key, uint16_t,
                                                  LRB3Compress_::hash, LRB3Compress_::Eq>>(
                SystemConf::getInstance().HASH_TABLE_SIZE));
      }
    }
    if (SystemConf::getInstance().CHECKPOINT_ON && SystemConf::getInstance().CHECKPOINT_COMPRESSION) {
      m_application->getCheckpointCoordinator()->setCompressionFP(0, LRB3Compress::compress);
      m_application->getCheckpointCoordinator()->setCompressionFP(1, LRB3Compress_::compress);
      m_application->getCheckpointCoordinator()->setDecompressionFP(0, LRB3Compress::decompress);
      m_application->getCheckpointCoordinator()->setDecompressionFP(1, LRB3Compress_::decompress);
    }
  }
